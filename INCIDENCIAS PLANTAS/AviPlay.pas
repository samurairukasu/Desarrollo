...
type
  ...
  TDCAVIPlay = class(TCustomControl)
  private
    fActive       : boolean;
    fAutoSize     : boolean;
    fCenter       : boolean;
...
procedure AVIFileInit; stdcall; external 'avifil32.dll' index 15;

procedure AVIFileExit; stdcall; external 'avifil32.dll' index 10;
...
function  AVIStreamOpenFromFile(avistream : PAVIStream; filename : pchar;
                                ...
                                mode : longint; clsid : pointer) : integer; stdcall; external 'avifil32.dll' index 48;

procedure AVIStreamRelease(avistream : PAVIStream); stdcall; external 'avifil32.dll' index 53;
function  AVIFileInfo(pfile : PAVIFile; pfi : PAVIFileInfo; lSize : longint) : integer; stdcall; external 'avifil32.dll' index 13;
...
function  AVIStreamRead(pavi : PAVIStream; lStart, lSamples : longint;
                        ...
                        plBytes,  plSamples : pointer) : integer; stdcall; external 'avifil32.dll' index 50;

function  AVIStreamReadFormat(pavi : PAVIStream; lPos : longint;
                              lpFormat : pointer; lpcbFormat : pointer) : integer; stdcall; external 'avifil32.dll' index 52;

function  AVIStreamBeginStreaming(pavi : PAVIStream; lStart, lEnd, lRate : longint) : integer; stdcall; external 'avifil32.dll' index 36;
function  AVIStreamEndStreaming(pavi : PAVIStream) : integer; stdcall; external 'avifil32.dll' index 38;
function  AVIStreamStart(pavi : PAVIStream) : longint; stdcall; external 'avifil32.dll' index 56;
function  AVIStreamLength(pavi: PAVIStream) : longint; stdcall; external 'avifil32.dll' index 46;
function  AVIStreamSampleToTime(pavi : PAVIStream; lSample : longint) : longint; stdcall; external 'avifil32.dll' index 54;
function  AVIStreamTimeToSample(pavi : PAVIStream; Time : longint) : longint; stdcall; external 'avifil32.dll' index 57;

function  DrawDIBOpen : THandle; stdcall; external 'msvfw32.dll' index 10;
procedure DrawDIBClose (h : THandle); stdcall; external 'msvfw32.dll' index 5;
procedure DrawDibDraw (hdib, dc : THandle; xDst, yDst, dxDst, dyDst : integer;
                       lpbi, lpBits : pointer; xSrc, ySrc, dxSrc, dySrc, wFlags : integer); stdcall; external 'msvfw32.dll' index 6;
...
constructor TDCAVIPlay.Create (AOwner : TComponent);
begin
...
procedure TDCAVIPlay.CreateParams(var Params: TCreateParams);
begin
...
function ThreadShowFrame(l : longint) : longint; stdcall;
begin
  with TDCAVIPlay(l) do
    ShowFrame;
...
begin
  with TDCAVIPlay(dwUser) do
    begin
      UpdateFrameNumber;
      inc(fFrame);
      if not fdrawing then
        PostMessage(Handle, WM_NEXTFRAME, 0, 0);
    end;
end;

{------------------------------------------------------------------}

procedure TDCAVIPlay.PlayAudio(startframe, endframe : integer);
var
  astart : integer;
  aend   : integer;
begin
...
procedure TDCAVIPlay.Play (FromFrame, ToFrame: Word; Count: Integer);
var
  info   : TAVIStreamInfo;
...
procedure TDCAVIPlay.Seek (Frame : integer);
begin
...
procedure TDCAVIPlay.SetActive(val : boolean);
begin
  if val = fActive then
    exit;
...
procedure TDCAVIPlay.SetAutoSize(val : boolean);
begin
...
procedure TDCAVIPlay.SetCenter(val : boolean);
begin
  if val = fCenter then
    exit;
...
procedure TDCAVIPlay.SetFileName(val : string);
var
  wasactive : boolean;
begin
...
procedure TDCAVIPlay.SetRepetitions(val : integer);
begin
...
procedure TDCAVIPlay.ValidateFrameNumber(var val : integer);
begin
  if fOpen then
...
procedure TDCAVIPlay.SetStartFrame(val : integer);
begin
...
procedure TDCAVIPlay.SetStopFrame(val : integer);
begin
...
procedure TDCAVIPlay.SetTransparent(val : boolean);
begin
...
procedure TDCAVIPlay.SetStretch (val : boolean);
begin
...
procedure TDCAVIPlay.SetPlaySound (val : boolean);
begin
...
procedure TDCAVIPlay.AdjustSize;
var
  info  : TAVIStreamInfo;
  r     : TRect;
  r2    : TRect;
...
procedure TDCAVIPlay.CloseFile;
begin
  if not fOpen then
    exit;
...
procedure TDCAVIPlay.CalcFrameLayout;
begin
  fxstart := 0;
  fystart := 0;
  if csDesigning in ComponentState then
...
procedure TDCAVIPlay.ShowFrame;
var
  image        : pointer;
  dc           : THandle;
  memdc        : THandle;
  ...
  brush        : THandle;
begin
  if fdrawing or ([csReading, csLoading] * ComponentState <> []) or (Parent = nil)
    or ([csReading, csLoading] * Parent.ComponentState <> [])
   or not HandleAllocated or not (visible or (csDesigning in ComponentState)) then
     exit;
  ...
  if fTransparent then
    begin
      ...
      if Assigned(image) then
        TransparentBitBlt(memdc, formdc, Rect(0, 0, width, height), Point(fxstart, fystart),
...
procedure TDCAVIPlay.UpdateFrameNumber;
begin
...
procedure TDCAVIPlay.PlayNextFrame(var Msg : TMessage);
begin
  ShowFrame;
end;
...
procedure TDCAVIPlay.SaveBackground;
var
  dc         : THandle;
  formdc     : THandle;
  oldfbitmap : THandle;
...
procedure TDCAVIPlay.WMPaint(var Msg : TMessage);
var
  ps : TPaintStruct;
begin
...
procedure TDCAVIPlay.WMEraseBkgnd(var Msg : TMessage);
var
  brush        : THandle;
  r            : TRect;
  dc           : THandle;
...
procedure TDCAVIPlay.WMMove (var Msg : TMessage);
begin
  inherited;
  fbackchanged := true;
  ShowFrame;
...
procedure TDCAVIPlay.KillTempDC;
begin
  if ftempdc <> 0 then
    begin
...
procedure TDCAVIPlay.WMSize (var Msg : TMessage);
begin
  fdrawing := true;
  KillTempDC;
  inherited;
...
var
  ...
  begin
    ...
    if (c <> nil) and not (c is TDCAVIPlay) and
       TDCAVIPlay(hooks[i]).HandleAllocated and
       (TDCAVIPlay(hooks[i]).owner = c.owner) or
       (TDCAVIPlay(hooks[i]).owner = c) then
          begin
            GetWindowRect(msg.wnd , r);
            GetWindowRect(TDCAVIPlay(hooks[i]).handle, r2);
...
begin
  ...
  if ((msg.message > CN_BASE) and (msg.message < CN_BASE + 500)) or
      (msg.message = WM_PAINT) or (msg.message = WM_SIZE) or
      (msg.message = WM_ERASEBKGND) then
    for i := 0 to hooks.Count - 1 do
      with TDCAVIPlay(hooks[i]) do
        if Transparent and IsPaintMsg then
          begin
            fbackchanged := true;
            r := ClientRect;
            InvalidateRect(TDCAVIPlay(hooks[i]).Handle, @r, true);
          end;
end;

{------------------------------------------------------------------}

procedure AddHook(o : TDCAVIPlay);
begin
...
procedure RemoveHook(o : TDCAVIPlay);
begin
  hooks.Remove(o);
  if hooks.Count = 0 then
...
procedure TDCAVIPlay.HookWndProc;
begin
  AddHook(self);
end;
...
procedure TDCAVIPlay.UnHookWndProc;
begin
  RemoveHook(self);
end;
...
procedure TDCAVIPlay.MMWOM_DONE(var M:TMessage);
begin
...
procedure TDCAVIPlay.DisplayChange(var Msg : TMessage);
begin
  DrawDIBClose(hdrawdib);
  hdrawdib := DrawDIBOpen;
end;
  ...
  with dialog do
    try
      ...
      if dialog.Execute then
        begin
          TDCAVIPlay(GetComponent(0)).FileName := dialog.FileName;
          Designer.Modified;
        end;
    finally
      Free;
...
begin
  result := TDCAVIPlay(GetComponent(0)).FileName;
end;
...
begin
  TDCAVIPlay(GetComponent(0)).FileName := val;
  Designer.Modified;
end;
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(string), TDCAVIPlay, 'FileName', TFileNameEdit);
end;
