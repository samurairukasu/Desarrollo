...
begin
  ...
  try
    ...
    f := f+2;
     ...
     with tsqlquery.Create(application) do
      try
        ...
        sql.Add('FROM TFACTURAS f, TCLIENTES c, TTIPOSCLIENTE tt, TFACT_ADICION a ') ;
        sql.Add('WHERE c.codclien = f.codclien and a.codfact=f.codfactu ')  ;
    ...
    with tsqlquery.Create(application) do
      try
        SQLConnection := mybd;
        sql.Add('SELECT codclien,DOCUMENT,SUM(subtotal), SUM(ivatotal),SUM(iibbtotal),SUM(total) FROM TTMP_LIQDIARIA_CLIENTCTACTE GROUP BY codclien, document');
        Open;
        f := 3;
        First;
        while not Eof  do
