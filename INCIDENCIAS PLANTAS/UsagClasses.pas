...
//            constructor CreateDescFromFact (const aBD: TSQLConnection; const aFactura: TFacturacion);
        ...
        public
            ...
            function VerImprimirInformeMedicion (const aEjercici,aCodinspe: integer; aContexto: pTContexto; const bVisualizar: boolean): boolean;
            procedure AfterPost(aDataSet: TDataSet); override;
            Procedure BeforePost(aDataSet: TDataSet); override;
...
var
    INet, IvaIns, IvaNIns, iibb: Double;
begin
{$IFNDEF SAT98}
  try
    Inet := StrToFloat(ValueByName[FIELD_IMPONETO]);
    iibb := strtofloat(ValueByName[FIELD_IIBB])/100*strtofloat(ValueByName[FIELD_IMPONETO]);
    if (ValueByName[FIELD_TIPFACTU] = FACTURA_TIPO_A) then
      begin
        ...
        result := Inet +  (StrToFloat(FloattoStrF((Inet*IvaIns*0.01),fffixed,8,2)) + StrToFloat(FloatTostrF((Inet*IvaNIns*0.01),fffixed,8,2)) )+iibb;
      end
    else
      result := Inet+iibb;
  except
    ...
    try
        ...
        try
          ...
          aQ.SQL.Add (Format('SELECT GETIMPONETO(%d,%d,%d) FROM DUAL',[aEjercicio,aCodigo,aCliente_ID]));
            ...
            self.edit;
            ValueByName[FIELD_CODFACTU] ;
            ValueByName[FIELD_IIBB] := aQ.Fields[0].AsString;
            self.Post(True);
        finally
            aQ.Close;
            aQ.Free;
...
(******************************************************************************)
    ...
    function TInspeccion.VerImprimirInformeMedicion (const aEjercici,aCodinspe: integer; aContexto: pTContexto; const bVisualizar: boolean): boolean;
    var
        ...
        try
            ...
            then begin
                try
                    ...
                    Result := Format('%1.2d %1.4d%1.4d%1.6d %s',[StrToInt(Copy(ValueByName[FIELD_EJERCICI],3,4)), fVarios.Zona, fVarios.CodeEstacion, StrToInt(ValueByName[FIELD_CODINSPE]), cV]);
...
//        SQL.ADD('AND V.CODVEHIC = I.CODVEHIC');
        open;
        result:=fields[0].asstring+' '+formatoceros(fVarios.Zona,4)+formatoceros(fVarios.CodeEstacion,4)+formatoceros(fields[1].asinteger,6);
      finally
        close;
        free;
      end;
