...
type
  TForm1 = class(TForm)
    ...
    procedure RegistroUpdateClick(Sender: TObject);
  private
 ...
 {Tomar los datos de la tabla reservas del SQL}
         ...
         if Existe=0 then
           begin
                 sqloracle:='INSERT INTO TDATOSTURNO (TURNOID,FECHATURNO,HORATURNO,FECHAREGISTRO,TTULARTIPODOCUMENTO,TITULARNRODOCUMENTO, '+
                            ...
                            ')';
                end else begin  //si ya existe, actualizar fecha,hora y estado del turno
                MEMO1.Lines.Add('Actualizando el Turno: '+INTTOSTR(TURNOID));
                RxTrayIcon1.HiNT:='Actualizando el Turno: '+INTTOSTR(TURNOID);
                APPLICATION.ProcessMessages;

                 sqloracle:='UPDATE TDATOSTURNO SET '+
                            ' FECHATURNO = TO_DATE('+#39+TRIM(FECHATURNO)+#39+',''DD/MM/YYYY'')'+
                            ',HORATURNO = '+#39+TRIM(HORATURNO)+#39+
                            //',FECHANOVEDAD = TO_DATE('+#39+TRIM(FECHANOVEDAD)+#39+',''DD/MM/YYYY'')'+
                            ',ESTADOID = '+INTTOSTR(ESTADOID)+
                            ',ESTADODESC = '+#39+TRIM(ESTADODESC)+#39+
      ...
      //    InsertaVehiculoNoMTOP(PATENTE,TIPO_VEHICULO);
       ...
       //IF (DEPARTAMENTAL = 0) AND NOT (TIPO_VEHICULO IN [7,8,9]) THEN BEGIN
          ...
          END ELSE BEGIN
          {Si el WS no devuelve datos, insertamos el vehiculo con datos basicos}
              InsertaVehiculoNoMTOP(PATENTE,TIPO_VEHICULO);
          END;
...
CLOSEFILE(ARCHI);
     ...
     begin
         ...
         LINEA_NUEVA:=copy(LINEA_NUEVA,posi,LARGO);
          ...
          HASTA:=posi1 - LARGO;
          ExpedicionITV:=trim(copy(LINEA_NUEVA,largo+1,HASTA-1));
      end;
     ...
     begin
       ...
       if pos('<Tag>',LINEA_NUEVA) > 0 then
     begin
         posi:=pos('<Tag>',LINEA_NUEVA);
         largo:=length(LINEA_NUEVA);
         LINEA_NUEVA:=copy(LINEA_NUEVA,posi,LARGO);

          posi:=pos('<Tag>',LINEA_NUEVA);
          largo:=length('<Tag>');
          posi1:=pos('</Tag>',LINEA_NUEVA);
          HASTA:=posi1 - LARGO;
          Tag:=trim(copy(LINEA_NUEVA,largo+1,HASTA-1));
      end;
     ...
     begin
      ...
      if pos('<Estado>',LINEA_NUEVA) > 0 then
     begin
         posi:=pos('<Estado>',LINEA_NUEVA);
         largo:=length(LINEA_NUEVA);
         LINEA_NUEVA:=copy(LINEA_NUEVA,posi,LARGO);

          posi:=pos('<Estado>',LINEA_NUEVA);
          largo:=length('<Estado>');
          posi1:=pos('</Estado>',LINEA_NUEVA);
          HASTA:=posi1 - LARGO;
          Estado:=trim(copy(LINEA_NUEVA,largo+1,HASTA-1));
      end;

      if pos('<Situacion>',LINEA_NUEVA) > 0 then
     begin
         posi:=pos('<Situacion>',LINEA_NUEVA);
         largo:=length(LINEA_NUEVA);
         LINEA_NUEVA:=copy(LINEA_NUEVA,posi,LARGO);

          posi:=pos('<Situacion>',LINEA_NUEVA);
          largo:=length('<Situacion>');
          posi1:=pos('</Situacion>',LINEA_NUEVA);
          HASTA:=posi1 - LARGO;
          Situacion:=trim(copy(LINEA_NUEVA,largo+1,HASTA-1));
      end;
...
procedure TForm1.RegistroUpdateClick(Sender: TObject);
BEGIN
 DoRegistroUpdate;
END;


procedure TForm1.DoRegistroUpdate;
VAR Existe:LONGINT;
PATENTE,CODVEHIC:STRING;
...
with tsqlQuery.Create(nil) do
    try
        ...
        SQL.Add('SELECT DISTINCT COUNT(*) '+
                'FROM TVEHICULOS '+
                'WHERE (NROREGISTRO IS NULL '+
                '   OR NROREGISTRO LIKE ''%-%'')'+
                ' AND PATENTEN IS NOT NULL');
        Open;
        Existe:=fields[0].ASInteger;
        finally
          Close;
...
with tsqlQuery.Create(nil) do
    try
        SQLConnection:= MYBD;
        SQL.Add('SELECT CODVEHIC, PATENTEN '+
                'FROM TVEHICULOS '+
                'WHERE (NROREGISTRO IS NULL '+
                '   OR NROREGISTRO LIKE ''%-%'') ' +
                ' AND PATENTEN IS NOT NULL');
        ...
        BEGIN
        PATENTE:=TRIM(FIELDBYNAME('PATENTEN').ASSTRING);
        CODVEHIC:=TRIM(FIELDBYNAME('CODVEHIC').ASSTRING);

        {Consulto patente al WS}
        limpiar_variables;
        WEBSERVICES_EMTO(PATENTE);
          ...
          IF (PaisISO <> '') OR (Matricula <> '') THEN BEGIN
             with tsqlQuery.Create(nil) do
               try
               SQLConnection:= MYBD;
               SQL.Add('UPDATE TVEHICULOS '+
                       'SET NROREGISTRO = ' +TRIM(Registro)+
                       ' WHERE CODVEHIC = ' +TRIM(CODVEHIC)+     //+IntToStr(CODVEHIC)+
                       '   AND (NROREGISTRO IS NULL '+
                       '     OR NROREGISTRO LIKE ''%-%'')');
                       //'   AND PATENTEN = ' +Matricula);
               ExecSQL;
           ...
           EXCEPT
             on E: Exception do
                      BEGIN
                         ...
                         APPLICATION.ProcessMessages;
                         GUARDA_LOG(DATETOSTR(DATE)+'|'+TIMETOSTR(TIME)+'| Registro: '+TRIM(PATENTE)+' NO SE PUDO ACTUALIZAR POR :'+ E.message);
                      END;
           END;

         END ELSE
           BEGIN
               //MEMO1.Lines.Add('Registro: '+TRIM(PATENTE)+ ' YA FUE ACTUALIZADO ANTERIORMENTE' );
               //RxTrayIcon1.HiNT:='Registro: '+TRIM(PATENTE)+ ' YA FUE ACTUALIZADO ANTERIORMENTE' ;
               //APPLICATION.ProcessMessages;
           END;
        limpiar_variables;
        NEXT;
  ...
  END ELSE
           BEGIN
               MEMO1.Lines.Add('NO HAY REGISTROS PARA ACTUALIZAR' );
               RxTrayIcon1.HiNT:='NO HAY REGISTROS PARA ACTUALIZAR' ;
               APPLICATION.ProcessMessages;
           END;
end;
