...
uses
  ...
  IdBaseComponent, IdComponent, IdUDPBase, IdUDPClient, OleServer, ExcelXP;
...
type
  ...
  public
    ...
    tx:txml_caba; TIENE_CONEXION_ELECTRONICA:BOOLEAN;
    ...
    function generar_pdf(nro_comprobante,cae,fechavence,gravado,iva21,total,TIPOCOMPROBANTE,PV:string;idturno:longint;letra,
                         patente_para_factura,NOMBRE_CLIENTE,TTIPODOCU,nro_doc:string;codclientefacturacion:LONGINT;MODO:STRING;iddetallespago:longint):STRING;
...
letra,patente_para_factura,NOMBRE_CLIENTE,TTIPODOCU,nro_doc:string;codclientefacturacion:LONGINT;MODO:STRING;iddetallespago:longint):STRING;
...
// facturabelectronica.QRLabel68.Caption:='0.00';
 ...
 facturabelectronica.QRLabel10.Caption:=datetostr(date);
 ...
 { aqQm:=tsqlquery.create(nil);
  ...
  END;
   ...
   begin
    ...
    facturabelectronica.QRLabel38.Caption:='"147 - TELÉFONO GRATUITO CABA, AREA DE DEFENSA Y PROTECCIÓN AL CONSUMIDOR"';
...
idturno:longint;letra,patente_para_factura,NOMBRE_CLIENTE,TTIPODOCU,nro_doc:string;codclientefacturacion:LONGINT;MODO:STRING;iddetallespago:longint):STRING;
var posi,I:longint;
...
{ frmdiseniofactelectronica.RxMemoryData1.Post; }
 ...
 frmdiseniofactelectronica.QRLabel10.Caption:=datetostr(date);
        ...
        BEGIN
           ...
           TX.Abrir_Seccion;
              ...
              BEGIN
               ...
               TX.DESCARGAR_FERIADO(TRIM(EDIT1.Text));
...
repeat
    ...
    ip:=Hoja.Range['H'+si,'H'+si].Value2; // Código
    if trIm(ip)<>'' then
       procesar_idpago_bajar(strtoint(ip)) ;
...
aqDTURNOS.Close;
aqDTURNOS.SQL.Clear;
aqDTURNOS.SQL.Add(' INSERT INTO  TDATOSTURNOHISTORIAL '+
...
IF TX.ver_respuestaidservidor=1 THEN
 ...
 BEGIN
 MEMO1.Clear;
MEMO1.Lines.Add('INICIO DESCARGA DE TURNOS DEL DÍA '+DATETOSTR(DATE));
...
'¡Felicitaciones! Ya abonaste el trámite de la VTV. Te enviamos el comprobante del pago correspondiente a tu vehículo '+trim(patente)+'.'+#13+#13+#13+
'Si tenés que cambiar o cancelar tu turno, entrá a http://www.buenosaires.gob.ar/tramites/vtv  .'+#13+#13+#13+
'¡Te esperamos! ';
...
if FORM1.CONEXION_OK=TRUE then
 BEGIN
  nombre_archivo:='';
      ...
      {generar pdf}
       if trim(letra)='A' then
          archivo:=generar_pdf(NRO_COMPROBANTE,FA.cae,FA.fecha_vence,IMP_NETO,IVA21,imp_total,STIPOCOMPROBANTE,INTTOSTR(punto_vta),iddetallespago,letra,patente_para_factura,nombre_para_mail,DFTIPODOCUMENTO,nro_doc,codclientefacturacion,fa.MODO_FAE,iddetallespago)
         else
         archivo:=generar_pdf_B(NRO_COMPROBANTE,FA.cae,FA.fecha_vence,IMP_NETO,IVA21,imp_total,STIPOCOMPROBANTE,INTTOSTR(punto_vta),iddetallespago,letra,patente_para_factura,nombre_para_mail,DFTIPODOCUMENTO,nro_doc,codclientefacturacion,fa.MODO_FAE,iddetallespago);
...
IVA21:=IMP_IVA;
    ...
    {generar pdf}
    if trim(letra)='A' then
       archivo:=generar_pdf(NRO_COMPROBANTE,CAEFACTURA,FECHAVENCEFACTURA,IMP_NETO,IVA21,imp_total,STIPOCOMPROBANTE,INTTOSTR(punto_vta),iddetallespago,letra,patente_para_factura,nombre_para_mail,DFTIPODOCUMENTO,nro_doc,codclientefacturacion,fa.MODO_FAE,iddetallespago)
    else

       archivo:=generar_pdf_B(NRO_COMPROBANTE,CAEFACTURA,FECHAVENCEFACTURA,IMP_NETO,IVA21,imp_total,STIPOCOMPROBANTE,INTTOSTR(punto_vta),iddetallespago,letra,patente_para_factura,nombre_para_mail,DFTIPODOCUMENTO,nro_doc,codclientefacturacion,fa.MODO_FAE,iddetallespago);
...
VAR AQCOMPROBANTES,aqtfacturas:tsqlquery;
iddetallespago,idpago:LONGINT;
BEGIN
 if FORM1.CONEXION_OK=TRUE then
begin
TRY
...
BEGIN
self.Caption:='DESCARGAR_TUNRO_SUVTV [6.0.0] v2  modo servidor';
   ...
   EDIT2.Enabled:=TRUE;
END ELSE BEGIN
self.Caption:='DESCARGAR_TUNRO_SUVTV [6.0.0] v2 modo descarga';
...
if (trim(timetostr(time))='07:00:00') then
  begin
  tx:=txml_caba.Create;
  tX.TestOfBD('', '', '' ,false);
...
BEGIN
SHOWMESSAGE('DEBE INGRESAR EL TURNOID');
EXIT;
END;

IDTURNO:=STRTOINT(EDIT1.Text);
TIMENOVEDAD.Enabled:=FALSE;
Factura_Electronica_POR_TURNO(IDTURNO);
...
IF TRIM(EDIT2.Text)='' THEN
 BEGIN
     ...
     Application.MessageBox( 'Debe ingrear la patente.', 'Atención',
  MB_ICONINFORMATION );
  exit;
 END;
 ...
 BEGIN
     ...
     Application.MessageBox( 'Debe ingrear el turnoid.', 'Atención',
  MB_ICONINFORMATION );
  exit;
 END;
...
IF TX.ver_respuestaidservidor=1 THEN
  ...
  MB_ICONINFORMATION );
         ...
         BEGIN
               ...
               Application.MessageBox('SERVIDOR SUVTV DISPONIBLE: NO', 'Atención',
  MB_ICONINFORMATION );
  ...
  END ELSE BEGIN
  Application.MessageBox(PCHAR('CONEXION A SUVTV: '+TX.ver_respuestamensajeservidor), 'Atención',
  MB_ICONINFORMATION );
...
FA.Free;
 TX.Free;;
Application.MessageBox( 'LISTO !!!.', 'Atención',
  MB_ICONINFORMATION ); }
