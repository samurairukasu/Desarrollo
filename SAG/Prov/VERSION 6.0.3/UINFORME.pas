            ...
            //29/09/2010
            {
             ...
             begin
               ...
               try
                 aq.SQLConnection := MyBD;
                 if Assigned(fInspeccion) then
                    aq.sql.add(format('select f.tipfactu, '+
                                      'case when f.tipfactu=''B'' then round(f.IMPONETO,2) else round((f.IMPONETO*(1 + (f.IVAINSCR / 100))),2)  end,   '+
                                      'round(((case when f.tipfactu=''B'' then round((f.IMPONETO/(1 + (f.IVAINSCR / 100))),2) else round(f.imponeto,2) end)*f.iibb/100),2) as IIBB_Prov,    '+
                                      'round(((case when f.tipfactu=''B'' then round((f.IMPONETO/(1 + (f.IVAINSCR / 100))),2) else round(f.imponeto,2) end)*f.iibb_CABA/100),2) as IIBB_CABA    '+
                                      'from tfacturas f, tinspeccion i where f.codfactu=I.CODFACTU and I.EJERCICI=%d and I.CODINSPE=%d ',[fEjercicio,fInspeccion.ValueByName[FIELD_CODINSPE]]))
                 else
                 aq.sql.add(format('select f.tipfactu, '+
                                      'case when f.tipfactu=''B'' then round(f.IMPONETO,2) else round((f.IMPONETO*(1 + (f.IVAINSCR / 100))),2)  end,   '+
                                      'round(((case when f.tipfactu=''B'' then round((f.IMPONETO/(1 + (f.IVAINSCR / 100))),2) else round(f.imponeto,2) end)*f.iibb/100),2) as IIBB_Prov,    '+
                                      'round(((case when f.tipfactu=''B'' then round((f.IMPONETO/(1 + (f.IVAINSCR / 100))),2) else round(f.imponeto,2) end)*f.iibb_CABA/100),2) as IIBB_CABA    '+
                                      'from tfacturas f, tinspeccion i where f.codfactu=I.CODFACTU and  I.EJERCICI=%d and I.CODINSPE=%d  ',[fEjercicio,fcodigo]));
               ...
               {  if  aq.Fields[0].asstring='B' then
                   ...
                   qrlPreve1.caption := 'USTED HA ABONADO POR ESTA INSPECCION : $'+formatfloat('0.00',aq.Fields[1].asfloat+aq.Fields[2].asfloat+aq.Fields[3].asfloat);
