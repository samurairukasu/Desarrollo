...
uses
  ...
  USAGCLASSES,UtilOracle,USAGVARIOS, uUtils, Provider, DBClient, ADODB, uinforme;
    ...
    if FinalizarInspeccion then
     ...
     ////armar consulta
          with aQ do
            begin
              SQLConnection:=mybd;
              SQL.Add ('SELECT COUNT(*) FROM TDATINSPECC WHERE EJERCICI = :EJERCICI AND ');
          ...
          begin
            if not LabelRechazada.Visible then
              begin
                ...
                if ((imprime_certificado=true) and (fVarios.ActivarCertificados = OP_ACTIVA)) then
                //   if fVarios.ActivarCertificados = OP_ACTIVA then       imprime certificado
                      begin

                        if (TrabajoEnviadoOk (UnaCabecera, Certificado))  and (TrabajoEnviadoOk (UnaCabecera, Informe)) then
                            begin
                              if (fInspeccion.ValueByName[FIELD_TIPO][1] in [T_NORMAL, T_VOLUNTARIA]) and (fVarios.ActivarInfMediciones = OP_ACTIVA) and  (imprimirMed=true)  then
    ...
    try
        ...
        case Resultado of
            ...
            begin
                ...
                LabelVencimiento.Caption := FormatDateTime('dd/mm/yyyy', dNuevaFechaDeVencimiento  (StrToInt(FInspeccion.ValueByName[FIELD_CODVEHIC]), FDataBase, INSPECCION_APTA,Tipo,StrToInt(FInspeccion.ValueByName[FIELD_CODINSPE]),StrToInt(FInspeccion.ValueByName[FIELD_EJERCICI])));
            end;

            CONDICIONAL:
            begin
                LabelCondicional.Visible := True;
                LabelVencimiento.Caption := FormatDateTime('dd/mm/yyyy', dNuevaFechaDeVencimiento (StrToInt(FInspeccion.ValueByName[FIELD_CODVEHIC]), FDataBase, INSPECCION_RECHAZADO,Tipo,StrToInt(FInspeccion.ValueByName[FIELD_CODINSPE]),StrToInt(FInspeccion.ValueByName[FIELD_EJERCICI])));
            end;
