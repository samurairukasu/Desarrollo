...
implementation
  ...
  //  fIncidencias.PonAnotacion(TRAZA_FLUJO,1,FILE_NAME, 'DISMINUYO CURSOR '+inttostr(cursores)+' max '+inttostr(MaxCursores)+' '+copy(fsagquery.text,pos('FROM',fsagquery.Text)+5,12));
    fIncidencias.PonAnotacion(TRAZA_FLUJO,1,FILE_NAME, 'DISMINUYO CURSOR '+inttostr(cursores)+' max '+inttostr(MaxCursores));
...
//    fIncidencias.PonAnotacion(TRAZA_FLUJO,1,FILE_NAME, 'INCREMENTO CURSOR '+inttostr(cursores)+' max '+inttostr(MaxCursores)+' '+copy(fsagquery.text,pos('FROM',fsagquery.Text)+5,12));
    fIncidencias.PonAnotacion(TRAZA_FLUJO,1,FILE_NAME, 'INCREMENTO CURSOR '+inttostr(cursores)+' max '+inttostr(MaxCursores));
    fIncidencias.PonComponente(TRAZA_SQL,1,FILE_NAME,fSagQuery);
      {$ENDIF}
...
begin
  ...
  try
          ...
          begin
            if fSagQuery.CanModify then
              begin
                ...
                if fSagQuery.State in [dsedit, dsinsert] then
                  fSagQuery.Post;
    ...
    procedure TSagData.Delete;
    begin
      if Active then
      Begin
        fSagQuery.Delete;
        {$IFDEF TRAZAS}
        fTrazas.PonAnotacion(TRAZA_FLUJO,1,FILE_NAME, 'Borrado del Registro');
        fTrazas.PonComponente(TRAZA_SQL,1,FILE_NAME,fSagQuery);
        {$ENDIF}
      end;
    end;
    ...
    begin
      ...
      fSagDataset.GetMetadata := false;
      fSagDataset.NoMetadata := true;
      fSagDataset.ParamCheck := false;      
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            imprimiendo_nc.Label1.Caption:=fFieldRequired;
            imprimiendo_nc.show;
            Break;
          end;
        end;
