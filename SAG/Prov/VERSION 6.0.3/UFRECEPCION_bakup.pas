       ...
       fVehiculo.Append ;
        with TSQLQuery.Create(application) do
         try
            ...
            SQL.Add(format('SELECT NRO_MOTOR  FROM VEHICULOS_TOTALES_Z2 WHERE PATENTE_ACTUAL=''%s''',[fPatente.Patente]));
            Open;
            fVehiculo.ValueByName[FIELD_NUMMOTOR] := Fields[0].AsString;
            //fVehiculo.ValueByName[FIELD_NUMBASTI] :=Fields[1].AsString;
         finally
            Close;
            Free;
        end;
...
var
  usuarioant: integer;
  claveant, numoblea, idusuario, zona, planta:string;
...
if fVehiculo.EsMayorA20 then
  ...
  begin
    ...
    begin
      result := false;
      if not GetObleaPlanta(zona, planta, numoblea) then exit;
      result := true;
    end;
