...
uses
  ...
  uftmp, sqlExpr, DBXpress, dbclient,provider,USagVarios;
...
type
  ...
  private
    ...
    procedure ImprimirLineaCostoCero(ainspeccion: Tinspeccion);
...
//        btnCancelar.ModalResult := mrNone;
        ...
        begin
              ...
              fInspeccion.Open;
           Try
           if ((fInspeccion.ValueByName[FIELD_TIPO] =T_REVERIFICACION) or (fInspeccion.ValueByName[FIELD_TIPO]= T_VOLUNTARIAREVERIFICACION)) then
             begin
               try
                  aQ:=TSQLQuery.create(application);
                  aQ.SQLConnection:=MYBD;

                  aQ.SQL.Add('SELECT SQ_TFACTURAS_NUMFACTUN.nextval FROM dual');
                  aQ.Open;

                  edtNumeroFacturaReal.Text := FormatoCeros(strtoint(ffactura.ffactadicionales.valuebyname[FIELD_PTOVENT]),4)+'-'+aQ.Fields[0].asstring ;
                  finally
                  aQ.close;
                ...
                fFactura.VAlueByName[FIELD_NUMFACTU] := trim(copy(edtNumeroFacturaReal.Text ,6, 7));
       ...
       //modif mla
          ...
          begin
           fInspeccion:=fFactura.GetInspeccion;
                fInspeccion.Open;
                 Try
           if ((fInspeccion.ValueByName[FIELD_TIPO] <>T_REVERIFICACION) and (fInspeccion.ValueByName[FIELD_TIPO]<>T_VOLUNTARIAREVERIFICACION)) then
            AbrirNoFiscal
            else
             begin
    ...
    if PagaBien Then
        If fFactura.ValidateNumeroFactura(edtNumeroFacturaReal.Text) then
          begin
            ...
            try
              DialogsFont.Size := 15;
              if MessageDlg(Caption,'Confirmar cobro de factura: '+ #13 + Format('Factura nº: %s.',[edtNumeroFacturaReal.Text]) + #13 +
                                ...
                                begin
                                  DialogsFont.Size := aSize;
                                    if FacturaImpresa then
                                      ...
                                      begin
                                        raise Exception.Create('No se ha podido Imprimir la Nota de Crédito');
                                        fAnomalias.PonAnotacion(TRAZA_SIEMPRE,0,FICHERO_ACTUAL, 'No se ha podido mostrar la Nota de Crédito');
                                      end;
                                end
                              else
                                begin
                                  raise Exception.Create('No se ha podido mostrar la Nota de Crédito');
                                  fAnomalias.PonAnotacion(TRAZA_SIEMPRE,0,FICHERO_ACTUAL, 'No se ha podido mostrar la Nota de Crédito');
                                end
                            else
                              begin
                                raise Exception.Create('No se ha podido generar la Nota de Crédito');
                                fAnomalias.PonAnotacion(TRAZA_SIEMPRE,0,FICHERO_ACTUAL, 'No se ha podido generar la Nota de Crédito');
                              end;
                          except
                            on E: Exception do
                              begin
                                application.messagebox('No se pudo generar correctamente la Nota de Crédito.',pchar(caption),mb_ok+mb_applmodal+mb_iconerror);
                                fAnomalias.PonAnotacionFmt(TRAZA_SIEMPRE,0,FICHERO_ACTUAL, 'No se pudo generar correctamente la Nota de Crédito. %s', [E.message]);
                              end;
                          end;
                    finally
                      finspeccion.free;
         ...
         try
            ...
            begin
              ...
              try
                fInspeccion.Open;
                 registradiscap;
                if (strtofloat(Ffactura.valuebyname[FIELD_IMPONETO]) <> 0) then //or (fCliente.ValueByName['tipocliente_id']=TIPO_CLIENTE_DISCAPACITADO) then
                begin
...
{aca no entro}      if strtofloat(ffactura.ValueByName[FIELD_IMPONETO])<>strtofloat(ffactura.ffactadicionales.ValueByName[FIELD_IMPSINDES]) then //MULTI
                ...
                begin
                  //se sca la impresion de tcicker para las reves  martin  22.12.2015
                 if (fInspeccion.ValueByName[FIELD_TIPO]= T_REVERIFICACION) or (fInspeccion.ValueByName[FIELD_TIPO]= T_VOLUNTARIAREVERIFICACION)  then
                 begin
                 // mla 01/2016 reves
                 ...
                 if ((fCliente.ValueByName['tipocliente_id']=TIPO_CLIENTE_DISCAPACITADO)
                    or (fCliente.ValueByName['tipocliente_id']=TIPO_CLIENTE_MUNICIPAL)
                    or (fCliente.ValueByName['tipocliente_id']=TIPO_CLIENTE_BOMBERO))
                    and (fInspeccion.ValueByName[FIELD_TIPO]<> T_REVERIFICACION)
                    and (fInspeccion.ValueByName[FIELD_TIPO]<> T_VOLUNTARIAREVERIFICACION) then
                 begin

                       ImprimirLineaDNF(fInspeccion);
                       cerrobien:=CerrarDNF;
                 end
                 else if (strtofloat(Ffactura.valuebyname[FIELD_IMPONETO]) = 0) and (fInspeccion.ValueByName[FIELD_TIPO]<> T_REVERIFICACION)
                         and (fInspeccion.ValueByName[FIELD_TIPO]<> T_VOLUNTARIAREVERIFICACION) then
                 begin
                       ImprimirLineaCostoCero(fInspeccion);
                       cerrobien:=CerrarDNF;
                 end;
                ...
                try
                  ...
                  begin
                   ...
                   if ((fInspeccion.ValueByName[FIELD_TIPO] <>T_REVERIFICACION) and (fInspeccion.ValueByName[FIELD_TIPO]<>T_VOLUNTARIAREVERIFICACION)) then
                    ...
                    if ((tipofactu = FACTURA_NO_FISCAL)  and (((fInspeccion.ValueByName[FIELD_TIPO] =T_REVERIFICACION) or (fInspeccion.ValueByName[FIELD_TIPO]= T_VOLUNTARIAREVERIFICACION)))) then

                       fFactura.VAlueByName[FIELD_NUMFACTU] := trim(copy(edtNumeroFacturaReal.Text ,6, 7))

                    else
                        fFactura.VAlueByName[FIELD_NUMFACTU] := impfis.AnswerField_10;
                       //mla reves nro de secuencia nuevo
...
begin
   ...
   if (ainspeccion.ValueByName[FIELD_TIPO]= T_REVERIFICACION) or (ainspeccion.ValueByName[FIELD_TIPO]=T_VOLUNTARIAREVERIFICACION)
      then
          ...
          tipover:='V';
   l3extra:='Nro: '+tipover+Trim(Copy(ainspeccion.ValueByName[FIELD_EJERCICI],3,2))+'000'+fvarios.ValueByName[FIELD_ZONA]+'000'+fvarios.ValueByName[FIELD_ESTACION]+formatoceros(strtoint(ainspeccion.ValueByName[FIELD_CODINSPE]),LENGTH(ainspeccion.ValueByName[FIELD_CODINSPE])); //AGRE VAZ
...
begin
    ...
    if (fFactura.ValueByName[FIELD_TIPFACTU]='B') then
    ImporteGravado :=(strtofloat(fFactura.ValueByName[FIELD_IMPONETO])/(1+(strtofloat(fFactura.ValueByName[FIELD_IVAINSCR]) / 100)))
    else
    ImporteGravado :=strtofloat(fFactura.ValueByName[FIELD_IMPONETO]);
    ...
    begin
        ...
        tasa:=floattostrf(strtofloat(floattostrf(strtofloat(fFactura.ValueByName[FIELD_IIBB])/100*ImporteGravado,fffixed,15,2))*100,fffixed,4,0);
        impfis.SendInvoicePerception(descitem,tipoitem,tasa);
    end;


    if (fVarios.ValueByName[FIELD_ENA_PIB] = 'S') and (fFactura.ValueByName[FIELD_IIBB_CABA] <> '0') then
    begin
        descitem:=('IIBB CABA.');
        tipoitem:='O';
        //MLA 14-07-2009   cambio en la cantidad de digitos de salida de la tasa de  3 paso a  4.
        tasa:=floattostrf(strtofloat(floattostrf(strtofloat(fFactura.ValueByName[FIELD_IIBB_CABA])/100*ImporteGravado,fffixed,15,2))*100,fffixed,4,0);
        impfis.SendInvoicePerception(descitem,tipoitem,tasa);
    end;
...
procedure TFrmCaja.ImprimirLineaCostoCero(ainspeccion: Tinspeccion);
var i:integer;
...
begin
      ...
      try
         QColetilla.SQLConnection := MyBD;
         QColetilla.SQL.Add('SELECT descripcion FROM TDESCUENTOS ');
         QColetilla.SQL.Add(Format('WHERE CODDESCU= ''%S'' ',[ffactura.ffactadicionales.valuebyname[FIELD_CODDESCU]]));
         QColetilla.Open;
      ...
      textonf:=descColet;
      impfis.SendNoFiscalText(textonf);
      textonf:='BONIF:                            -'+ffactura.ffactadicionales.valuebyname[FIELD_DESCUENT];
      impfis.SendNoFiscalText(textonf);
      textonf:=LINEA_DNF;
      impfis.SendNoFiscalText(textonf);
...
begin
   try
      ...
      impfis.SetGetHeaderTrailer(accion,dfnumero,texto);
         ...
         ///********imprime encabezado  cambio de mb 15/04/2014
         //***************************************


           dfnumero:='6';   //6
          // texto:=trim(fvarios.Direccion);
           texto:=stringparacf(fdatosestacion.valuebyname[FIELD_CALLECOM]);
           Impfis.SetGetHeaderTrailer(accion,dfnumero,texto);

           dfnumero:='7';    //6
          // texto:=cVacio;
           texto:='('+stringparacf(fdatosestacion.valuebyname[FIELD_CODPOSTALCF])+') '+stringparacf(fdatosestacion.valuebyname[FIELD_LOCALIDAD]);
           Impfis.SetGetHeaderTrailer(accion,dfnumero,texto);
           ...
           dfnumero:='9';
           texto:='II BB:'+stringparacf(fdatosestacion.valuebyname[FIELD_INSINGBRUT]);
           Impfis.SetGetHeaderTrailer(accion,dfnumero,texto);

         //dfnumero:=ENCA_FECINI;
           dfnumero:='10';
           texto:='Inicio de Actividad:'+stringparacf(fdatosestacion.valuebyname[FIELD_FECHINIACT]);
           Impfis.SetGetHeaderTrailer(accion,dfnumero,texto);
 ...
 try
  ...
  with dCabecera do
    begin
      ...
      if (fFactDescuento.ValueByName[FIELD_TIPFACTU] = CADENA_FACTURA_B) then
        iServicio := NCDescuento_B;

       if (fFactDescuento.ValueByName[FIELD_TIPFACTU] = 'S') then
        iServicio := NCDescuento_B;
