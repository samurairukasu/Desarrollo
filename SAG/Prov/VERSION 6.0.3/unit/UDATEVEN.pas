    ...
    try
        ...
        try
            ...
            then begin
                ...
                then CommandText := (Format('SELECT TO_CHAR(FECVENCI, ''DD/MM/YYYY''), RESULTAD FROM TINSPECCION ' +
                                     ' WHERE (CODVEHIC = %D) AND (INSPFINA=''S'') AND (TIPO IN (''%S'',''%S'')) ' +
                                     ' ORDER BY FECHALTA DESC',[iCodigoVehiculo,T_NORMAL,T_REVERIFICACION]))

                else CommandText := (Format('SELECT TO_CHAR(FECVENCI, ''DD/MM/YYYY'') FROM TINSPECCION ' +
                                     ' WHERE (CODVEHIC = %D) AND (INSPFINA=''S'') AND (TIPO IN (''%S'',''%S'')) ' +
...
var
    ...
    var
     ...
     begin
        ...
        Result := rDividendo div NUM_DIAS_MES;
    end;
    ...
    //  result := ABS(F.dFechaActual - F.dFechaVencimientoApto) <=  DIAS_X_MES
    end;
...
begin
    try
      ...
      try
        ...
        begin
          CommandText:= (Format('SELECT TO_CHAR(FECVENCI, ''DD/MM/YYYY'') FROM TINSPECCION ' +
                        ' WHERE (CODVEHIC = %D) AND (INSPFINA=''S'') AND (TIPO IN (''%S'',''%S'')) AND (RESULTAD =''%S'' )' +
                        ' ORDER BY FECHALTA DESC',[iCodigoVehiculo,T_NORMAL,T_REVERIFICACION,INSPECCION_APTA]));
          ...
          begin
            ...
            close;
            SetProvider(dsp);
            CommandText:= (Format('SELECT TO_CHAR(FECVENCI, ''DD/MM/YYYY''),TO_CHAR(FECHALTA, ''DD/MM/YYYY''),RESULTAD FROM TINSPECCION ' +
            ' WHERE (CODVEHIC = %D) AND (INSPFINA=''S'') AND (TIPO IN (''%S''))' +
            ' ORDER BY FECHALTA DESC',[iCodigoVehiculo,T_NORMAL]));
            Open;
...
begin
  ...
  with consulta do
    try
      ...
      SQL.Add (Format('SELECT EJERCICI, CODINSPE FROM TINSPECCION ' +
            ' WHERE (CODVEHIC = %D) AND (INSPFINA=''S'') AND (TIPO IN (''%S''))' +
            ' ORDER BY FECHALTA DESC',[iCodigoVehiculo,T_NORMAL]));
      open;
