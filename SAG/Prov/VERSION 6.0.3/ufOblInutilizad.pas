  ...
  try
    with TSQLQuery.Create(Self) do
      begin
        ...
        SQL.Clear;
        Close;
        SQL.Add('SELECT * FROM TINSPECCION WHERE NUMOBLEA=:OBLEA and EJERCICI=:EJERCICI');
        ParamByName('OBLEA').AsString:=Oblea;
        ParamByName('EJERCICI').AsString:=EJERCICI;
        Open;
        if not IsEmpty then
          Result:=true
        else
...
{if aoblea.text = '' then
  Error:=Error+'- Año de oblea nueva.'+#13#10; }
...
if EstaAsignada(edobleanueva.Text, edano.Text) then    //edano
     ...
     fbusqueda.Temporizar(True,True,'Inutilizando Oblea','Espere por favor.');
      if (Error = '') then
        Begin
          ...
          aqnc:=TSQLQuery.Create(self);
          aqnc.SQLConnection:=mybd;
          aqnc.Close;
          aqnc.SQL.Clear;
          aqnc.SQL.Add('select F.CODCOFAC from tinspeccion i , tfacturas f '+
                       ' where I.CODINSPE=:CODINS and I.EJERCICI=:EJERC   '+
                       ' and I.CODFACTU=F.CODFACTU   '+
                      ' and F.CODCOFAC is not null ');
          aqnc.ParamByName('EJERC').AsInteger:=StrToInt(edano.Text);
          aqnc.ParamByName('CODINS').AsInteger:=StrToInt(ednroinspec.Text);
          aqnc.ExecSQL;
          aqnc.Open;
          tienenc:=false;
          while not aqnc.Eof do
             begin
               tienenc:=true;
              aqnc.Next;
           end;
             ...
             if tienenc=true then
             begin
             Screen.Cursor:=crDefault;
                Temporizar(false,false,'','');
                Application.MessageBox( 'No se puede inutilizar la oblea porque la inspección tiene una nota de credito asociada.',
...
//********************************** VERSION SAG 4.00 **********************************************
            ...
            // ***************************************************************************
            //23-12-2013  martin. se anulo la reimpresion de certificado y se puso para que
            //reimprima el informe
           // UReimpresion.ImprimirCertificado(sEjercici,sCodInsp,sMatricula);
            ureimpresion.Imprimirinmforme_cambio_oblea(sEjercici,sCodInsp,sMatricula);
            Imprimio:= true;
//**************************************************************************************************
