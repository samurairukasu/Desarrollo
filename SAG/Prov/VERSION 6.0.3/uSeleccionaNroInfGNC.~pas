unit uSeleccionaNroInfGNC;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  StdCtrls, Buttons, ExtCtrls, Db, Mask, FMTBcd, SqlExpr;

type
  TfrmSeleccionaNroInfGNC = class(TForm)
    Bevel1: TBevel;
    lblInforme: TLabel;
    btnAceptar: TBitBtn;
    btnCancelar: TBitBtn;
    edtInformeInspeccion: TMaskEdit;
  ...
  function SeleccInformeGNC(const iZona, iCodEstac: integer; var aEjercicio, aCodigoInspeccion:integer; var aFecVenci: string): boolean;

var
  frmSeleccionaNroInfGNC: TfrmSeleccionaNroInfGNC;
...
begin
    ...
    Result:=FalsE;
    try
        with tfrmSeleccionaNroInfGNC.Create(Application) do
        try
            if ShowModal = mrOk
            then begin
...
procedure TfrmSeleccionaNroInfGNC.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
...
// Devuelve True si el nº de informe de inspección introducido existe en TINSPECCION y la inspección está finalizada
function TfrmSeleccionaNroInfGNC.Existe_InformeInspeccion_TINSPECCION: boolean;
var
       ...
       begin
           try
              ...
              begin
                  Close;
                  Sql.Clear;
                  Sql.Add (Format('SELECT %s,%s,FECHVENCI FROM %s WHERE %s=%d and %s=%d and INSPFINA=''S''',[FIELD_EJERCICI, FIELD_CODINSPGNC, DATOS_INSPGNC, FIELD_EJERCICI, iEjercicio,FIELD_CODINSPGNC, iCodInsp]));
                  {$IFDEF TRAZAS}
...
procedure TfrmSeleccionaNroInfGNC.btnAceptarClick(Sender: TObject);
var
...
procedure TfrmSeleccionaNroInfGNC.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
    qryConsultas.Close;
end;

procedure TfrmSeleccionaNroInfGNC.FormCreate(Sender: TObject);
begin
    with qryConsultas do
    begin
        Close;
...
function TfrmSeleccionaNroInfGNC.Sumar_1900 (iAnio: integer): integer;
...
procedure TfrmSeleccionaNroInfGNC.edtInformeInspeccionEnter(
  Sender: TObject);
begin
    DestacarControl (Sender, clGreen, clWhite, False);
end;

procedure TfrmSeleccionaNroInfGNC.edtInformeInspeccionExit(
  Sender: TObject);
begin
    Text := Trim(Self.Text);
