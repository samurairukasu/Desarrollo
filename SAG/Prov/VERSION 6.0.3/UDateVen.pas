...
interface
 ...
 function dNuevaFechaDeVencimiento ( const iCodigoVehiculo : integer;aDataBase:tSQLConnection;
                                     const cResultadoInspeccionActual,aTipo: char;CODINSPE,EJERCICI:LONGINT) : Integer;
...
Function ObtieneFechaVTVVIGENTEExterna( const CODINSPE,EJERCICI,iCodigoVehiculo : integer;aDataBase:TSQLConnection; const cResultadoInspeccionActual:char) : ttFechasReve;
...
function tFechasVehiculo (const iCodigoVehiculo : integer; const aDataBase: TSQLConnection; const aTipo : char;const cResultadoInspeccionActual:char;CODINSPE,EJERCICI:LONGINT ) : ttFechas;
const
    UNICO = 1;
var
        ...
        try
            ...
            //Seleccionar fecha de ultima inspeccion valida
              ...
              Close;
               ...
               SetProvider(dsp);
               CommandText:= (Format('select * from insp_vigentes     WHERE CODINSPE=%d AND EJERCICI=%d and fechavenci is not null',[CODINSPE,EJERCICI])) ;
               Open;
                if Recordcount>0 THEN
                   existeinspvigente:=TRUE
                   ELSE
                   existeinspvigente:=FALSE;
            ...
            then begin
                ...
                SetProvider(dsp);
                if existeinspvigente  then
                                  CommandText:= (Format('select TO_CHAR(FECHAVENCI, ''DD/MM/YYYY''),''A'' from insp_vigentes   '+
                                           ' WHERE CODINSPE=%d AND EJERCICI=%d',[CODINSPE,EJERCICI]))
                else
                begin
...
begin
  try
    ...
    tFechas := tFechasVehiculo(iCodigoVehiculo,aDataBase,aTipo,cResultadoInspeccionActual,CODINSPE,EJERCICI);
...
var
    ...
    dFechaInspeccion: integer;
    FRECUENCIAS : ttFrecuencias;
    RESULTADODEOTRAPLANTA:sTRING;
begin
    try
    RESULTADODEOTRAPLANTA:='A';
      Result.dResultado:=cResultadoInspeccionActual;
      aQ := TSQLDataSet.Create(application);
      aQ.SQLConnection := aDataBase;
      aQ.CommandType := ctQuery;
      ...
      try
        ...
        if result.dresultado <> INSPECCION_APTA then  //
        begin
             CommandText:= (Format('select TO_CHAR(FECHAVENCI, ''DD/MM/YYYY'') from insp_vigentes   '+
                                  ' WHERE CODINSPE=%d AND EJERCICI=%d',[CODINSPE,EJERCICI]));
        ...
        begin
          ...
          begin
            ...
            SetProvider(dsp);
                  ...
                  CommandText:= (Format('select TO_CHAR(FECHAVENCI, ''DD/MM/YYYY''), TO_CHAR(FECHALTA, ''DD/MM/YYYY'') from insp_vigentes   '+
                                  ' WHERE CODINSPE=%d AND EJERCICI=%d',[CODINSPE,EJERCICI]));
            ...
            begin
                //mla if abs(dFechaInspeccion - Result.dFechaVencimientoApto) >  then
                 ...
                 begin
                      ...
                      if RESULTADODEOTRAPLANTA= INSPECCION_APTA then//MARTIN
                     // if FIELDS[2].ASSTRING = INSPECCION_APTA then  //   MODI RAN
                         Result.dResultado:=INSPECCION_APTA
                      else Result.dResultado:='C';
                 end;
          ...
          begin
            ...
            if Frecuencias.iPeriodicidad3 = 0 then
              ...
              if RESULTADODEOTRAPLANTA = INSPECCION_APTA then
           // if FIELDS[1].ASSTRING = INSPECCION_APTA then  //   MODI RAN
                  Result.dResultado:=INSPECCION_APTA
          // else if FIELDS[1].ASSTRING = INSPECCION_CONDICIONAL then
           else if RESULTADODEOTRAPLANTA = INSPECCION_CONDICIONAL then
                    Result.dResultado:=INSPECCION_CONDICIONAL
                else
                    Result.dResultado:='R';
             ...
             on E : Exception do
             begin
               fIncidencias.PonAnotacion(0,0,FICHERO_ACTUAL,'ERROR OTENIENDO LAS FECHAS DEL VEHICULO  VIGENTE EXTERNA REVERIFICADO ' + IntToStr(iCodigoVehiculo) + ' POR: '+ E.message);
               raise;
             end;
      end;
    finally
