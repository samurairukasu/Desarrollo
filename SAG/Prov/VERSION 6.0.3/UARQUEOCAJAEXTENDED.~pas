...
type
  ...
  public
    ...
    TotalIva,
    TotalIvaNoInscripto,
    Total, TotalIIBB, TotalIIBB_CABA,
    NumeroFacturas,
  ...
  procedure DoArqueoCaja(const FI,FF,sTipoArqueo: string;
                         var sTotFact, sTotNC, sTotImp, sTotIVAIns, sTotIVANoIns, sTotTot, sTotIIBB,sTotIIBB_CABA: string; sTipoCliente:string);
...
begin
    ...
    try
        ...
        sTotIIBB_CABA := floattostrf(strtofloat(ParamByName('IIBBCABAEnCaja').AsString),fffixed,8,2);
        Close;
    finally
        Free
    end
        ...
        try
            try
                ...
                if (sTipo_Auxi = TIPO_ARQUEO_CLIENTES) or (sTipo_auxi = TIPO_ARQUEO_TARJETA) then
                  DoArqueoCaja(DateIni, DateFin, TipoPago, NumeroFacturas, NumeroNotasCredito, TotalImporte, TotalIva, TotalIvaNoInscripto, Total, TotalIIBB,TotalIIBB_CABA, aTipoCliente)
                else
                  DoArqueoCaja(DateIni, DateFin, TipoPago, NumeroFacturas, NumeroNotasCredito, TotalImporte, TotalIva, TotalIvaNoInscripto, Total, TotalIIBB,TotalIIBB_CABA,'');
...
begin
    try
        ...
        EdTotalIIBB_CABA.Text := TotalIIBB_CABA;
...
//                    'DOMINIO, TIPOPAGO, TIPOCLIENTE, NOMBRECLIENTE, IMPORTE, IVA, IVANOINSCRIPTO, ');
            ...
            if sTipo_Auxi <>  TIPO_ARQUEO_CLIENTES then
              fsql.add('TOTAL, IIBB, IIBB_CABA, CONCEPTO FROM TTMPARQCAJAEXTENCAJ ORDER BY FO, TIPOFACTURA, PTOVENTA, NUMFACTURA ')
            else
              fsql.add('TOTAL, IIBB, IIBB_CABA, CONCEPTO FROM TTMPARQCAJAEXTEN ORDER BY FO, TIPOFACTURA, PTOVENTA, NUMFACTURA ');
       ...
       then begin
            case Imprimir of
                ...
                begin
                    ...
                    try
                        Execute(DateIni, DateFin, TotalImporte, TotalIva, TotalIvaNoInscripto, Total, NumeroFacturas, NumeroNotasCredito, PutTipoPago(TipoPago), NombreEstacion, NumeroEstacion, aTipoCliente, TotalIIBB,TotalIIBB_CABA );
                    finally
                        Free;
                    end;
                end;
       ...
       then begin
            case ExportarAscii of
                ...
                begin
                    with TFArqueoCajaExtendedToPrint.Create(Application) do
                    try
                        ExportaAscii(DateIni, DateFin, TotalImporte, TotalIva, TotalIvaNoInscripto, Total, NumeroFacturas, NumeroNotasCredito, PutTipoPago(TipoPago), NombreEstacion, NumeroEstacion, aTipoCliente, TotalIIBB,TotalIIBB_CABA);
                    finally
                        Free;
                    end;
                end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        ExcelHoja.Cells[f,C_TIBC].value := conviertecomaenpunto(EdTotaliibb_CABA.Text);
