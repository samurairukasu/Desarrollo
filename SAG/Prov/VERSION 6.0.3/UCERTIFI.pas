...
type
  ...
  public
    { Public declarations }
    VECTOR_CALIFI:ARRAY[1..2] OF STRING;
    ...
    constructor CreateFromEjercicioAndCode (const aEjercicio, aCode : longint; const bPresPreliminar: boolean; aContexto: pTContexto);
    FUNCTION GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO(CADENA:STRING):BOOLEAN;
  end;
...
// mb
FUNCTION TFrmCertificado.GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO(CADENA:STRING):BOOLEAN;
VAR
    POSI, POSI1:LONGINT;
    CALI1,CALI2:STRING;
begin
 VECTOR_CALIFI[1]:='';
 VECTOR_CALIFI[2]:='';
 POSI:=POS('HASTA',TRIM(CADENA));
 POSI1:=POS('MAS',TRIM(CADENA));
 IF POSI > 0   THEN
   BEGIN
        CALI1:=COPY(TRIM(CADENA),0,POSI-1);
        CALI2:=COPY(TRIM(CADENA),POSI,LENGTH(TRIM(CADENA)));
        VECTOR_CALIFI[1]:=TRIM(CALI1);
        VECTOR_CALIFI[2]:=TRIM(CALI2);
        GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO:=TRUE;
   ...
   BEGIN
         IF POSI1 > 0   THEN
          BEGIN
             CALI1:=COPY(TRIM(CADENA),0,POSI1-1);
             CALI2:=COPY(TRIM(CADENA),POSI1,LENGTH(TRIM(CADENA)));
              VECTOR_CALIFI[1]:=TRIM(CALI1);
              VECTOR_CALIFI[2]:=TRIM(CALI2);
              GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO:=TRUE;
              EXIT;
            END else
            begin
                  GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO:=FALSe;
            end;
...
begin
    ...
    try
        ...
        { TODO -oran -ctransacciones : ver tema session }
         SQL.Add (Format(
                   ' SELECT IMPRESION,CLASIFIC,ABRCLASI, NVL(PATENTEN, PATENTEA) PATENTE, (NOMBRE || '' '' || APELLID1 || '' '' || APELLID2) NOMBREC, (TIPODOCU ||'' '' || DOCUMENT) DOCUMENTO ' +
       ...
       //TRANSPORTE PUBLICO PERSONAS HASTA 9 PLAZAS
         VECTOR_CALIFI[1]:='';
         VECTOR_CALIFI[2]:='';
           //IF GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO(TRIM( FieldByName('CLASIFIC').AsString))=TRUE THEN
       IF GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO(TRIM( FieldByName('IMPRESION').AsString))=TRUE THEN
       begin
               LblClasificacion.Caption := TRIM( VECTOR_CALIFI[1]);
               LblClasificacion1.Caption := TRIM( VECTOR_CALIFI[2]);
           END ELSE
            BEGIN
             LblClasificacion.Caption := TRIM( FieldByName('IMPRESION').AsString);
             LblClasificacion1.Caption :='';
           END;
...
constructor TFrmCertificado.CreateFromEjercicioAndCode (const aEjercicio, aCode : longint; const bPresPreliminar: boolean; aContexto: pTContexto);
begin
    ...
    try
       ...
       // LblClasificacion.Caption := FieldByname(FIELD_ABRCLASI).AsString;
        //TRANSPORTE PUBLICO PERSONAS HASTA 9 PLAZAS
         ...
         VECTOR_CALIFI[2]:='';
         //IF GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO(TRIM( FieldByName('CLASIFIC').AsString))=TRUE THEN
       // IF GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO('TRANSPORTE PUBLICO PERSONAS HASTA 9 PLAZAS')=TRUE THEN
             IF GENERA_CADENA_CALIFICACION_PARA_CERTIFICADO(TRIM( FieldByName('IMPRESION').AsString))=TRUE THEN
       begin
               LblClasificacion.Caption := TRIM( VECTOR_CALIFI[1]);
               LblClasificacion1.Caption := TRIM( VECTOR_CALIFI[2]);
