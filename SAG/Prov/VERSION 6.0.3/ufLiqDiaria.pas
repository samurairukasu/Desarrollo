...
begin
        ...
        begin
            ...
            fsql.add('SUBTOTAL, IVATOTAL, IIBBTOTAL, IIBB_CABATOTAL,TOTAl FROM TTMP_LIQDIARIA_TOTFPAGO ORDER BY NRO');
...
begin
  ...
  try
    ...
    excelHoja2.cells[f,i+6].value := 'IIBB_CABA';
    excelHoja2.cells[f,i+7].value := 'Total';
    with tsqlquery.Create(application) do
      try
        SQLConnection := mybd;
        sql.Add('SELECT codclien,DOCUMENT,SUM(subtotal), SUM(ivatotal),SUM(iibbtotal),SUM(iibb_CABAtotal),SUM(total),nro FROM TTMP_LIQDIARIA_CLIENTCTACTE GROUP BY codclien, document, nro');
        Open;
        f := 3;
        First;
        while not Eof  do
             begin
               ...
               excelHoja2.cells[f,i+6].value := fields[6].asstring;
               excelHoja2.cells[f,i+7].value := fields[7].asstring;
               Next;
               inc(f);
             end ;
         finally
