...
{       System Unit                                     }
{                                                       }
{       Copyright (C) 1988,97 Borland International     }
{                                                       }
{*******************************************************}
...
var
  ...
  MainInstance: Longint;  { Handle of the main(.EXE) HInstance }
...
type
  ...
  TEnumModuleFunc = function (HInstance: Longint; Data: Pointer): Boolean;
  TModuleUnloadProc = procedure (HInstance: Longint);

  PModuleUnloadRec = ^TModuleUnloadRec;
  TModuleUnloadRec = record
    Next: PModuleUnloadRec;
    Proc: TModuleUnloadProc;
  end;
...
function RegOpenKeyEx(hKey: Integer; lpSubKey: PChar; ulOptions: Longint;
  samDesired: Longint; var phkResult: Integer): Longint; stdcall;
  external advapi32 name 'RegOpenKeyExA';

function RegQueryValueEx(hKey: Integer; lpValueName: PChar;
...
begin
  ...
  asm
    db 'Portions Copyright (c) 1983,97 Borland',0
  end;
...
const
  ...
  LocaleOverrideKey = 'Software\Borland\Delphi\Locales'; // do not localize
...
begin
  ...
  begin
    if Instance = CurModule.Instance then
    begin
...
begin
  ...
  LocaleOverride[0] := #0;
  if RegOpenKeyEx(HKEY_CURRENT_USER, LocaleOverrideKey, 0, KEY_ALL_ACCESS, Key) = 0 then
  try
...
procedure NotifyModuleUnload(HInstance: Longint);
var
  P: PModuleUnloadRec;
begin
  P := ModuleUnloadList;
