...
iDiasMes:= DaysInMonth(Now);
  try
    Result:= Abs((iCincoAleatorios - iDiasMes) * (iTresdeOblea + iDiaHoy));
  except
    On E: Exception Do
...
Resultado:= Abs((Clave - DiasMes) * (TresdeOblea + DiaHoy));
...
procedure TFrmRestaurarObleaPWD.BtnValidarClick(Sender: TObject);
begin
...
If (ENumAleatorios.Text <> '') and (EObleaDigitos.Text <> '') then
  BtnValidar.Caption:=IntToStr(GenerarClave(ENumAleatorios.Text, EObleaDigitos.Text));
end;
