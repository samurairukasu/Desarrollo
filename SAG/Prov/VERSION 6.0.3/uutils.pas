...
With TSQLQuery.Create(nil) do
  try
    ...
    Close;
    SQL.Clear;
    SQL.Add('Select MIN(NUMOBLEA), ESTADO FROM TOBLEAS WHERE ESTADO = :LIBRE AND ANIO = :ANIOVENCI GROUP BY ESTADO');
...
*)
   ...
   begin
       ...
       if  pos(' ',trim(sNumInsp)) =0 then
           sNumInsp:=Copy (sNumInsp, 1, 2)+' '+Copy (sNumInsp, 3,length(trim(sNumInsp)));

       try
          Result := StrToInt(Copy (sNumInsp, 12, length(sNumInsp)));
       except
            on E:Exception do
