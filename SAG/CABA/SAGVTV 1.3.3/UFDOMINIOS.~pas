...
interface
    ...
    procedure FormActivate(Sender: TObject);
        private
            ...
            function TIENE_TIENE_FACTURA(IDPAGOFERIFICACION:LONGINT):BOOLEAN;
...
{$R *.DFM}
    ...
    BEGIN
       ...
       CODTURNOID:=-1;
        ...
        varSql:='SELECT  MAX(TURNOID) AS TURNOID, codvehic,PAGOIDVERIFICACION, tipoturno FROM TDATOSTURNO  '+
                '  WHERE TO_CHAR(FECHATURNO,''DD/MM/YYYY'') = to_char(SYSDATE,''DD/MM/YYYY'')  and estadoid IN (1,7,3) and  DVDOMINO = '''+trim(PATENTE)+''' '+
    ...
    function TFDominios.TIENE_TIENE_FACTURA(IDPAGOFERIFICACION:LONGINT):BOOLEAN;
    var varSql:string;
    notacredito,tienefactura:boolean;
    BEGIN
     tienefactura:=false;
        ...
        with TSQLQuery.Create(application) do
         try
            SQLConnection:=MYBD;
            SQl.clear;
            varSql:='SELECT codcofac FROM TFACTURAS WHERE  IDPAGO='+INTTOSTR(IDPAGOFERIFICACION);
            SQL.ADD(varSql);
            Open;
             while not eof do
              begin
                tienefactura:=true;
                 if trim(fields[0].asstring)='' then
                  begin
                     TIENE_TIENE_FACTURA:=TRUE;
                     break;
                  end     ELSE
                      begin
                        Application.MessageBox( 'ESTE VEHICULO TIENE UNA NOTA DE CREDITO ASOCIADA.',
                                                 'ATENCION', MB_ICONSTOP );
                      TIENE_TIENE_FACTURA:=FALSE;
                      break;
                      end;
          ...
          if tienefactura=false  then
             begin
                 {forzar el ingreso por contraseña}
                 Application.MessageBox( 'ESTE VEHICULO NO HA SIDO FACTURADO. COMUNIQUESE CON SISTEMAS.','ATENCION', MB_ICONSTOP );
                 TIENE_TIENE_FACTURA:=TRUE;
             end;
    ...
    continua:=true;
        ...
        begin
             ...
             begin
                Application.MessageBox( 'ESTE VEHICULO NO TIENE TURNO PARA EL DÍA DE HOY',
               ...
               {CONTROLA QUE TENGA FACTURA ELECTRONICA Y NO TENGA NC}
               IF TIENE_TIENE_FACTURA(PAGOIDVERIFICACION_TURNO_INGRESO)=FALSE THEN
               begin
               ...
               with TSQLQuery.Create(application) do
                 try
                    SQLConnection:=MYBD;
                    sql.Add('select ti.codinspe from tinspeccion ti, tdatosturno td  '+
                            ...
                            ' and  TI.RESULTAD=''A''  and td.turnoid='+inttostr(CODTURNOID)+
                           ' and TI.CODVEHIC =TD.CODVEHIC and ti.codinspe=td.codinspe ') ;
                    Open;
                    while not eof do
                     begin
                           Application.MessageBox( 'ESTE VEHICULO YA  TIENE UNA INSPECCION FINALIZADA APROBADA EN ESTE DÍA',
                                        'Atención', MB_ICONSTOP );
               ...
               with TSQLQuery.Create(application) do
                 try
                    ...
                    sql.Add('select ti.codinspe,td.dvdomino from tinspeccion ti, tdatosturno td  '+
                            ' where TO_CHAR(ti.fechalta,''DD/MM/YYYY'') = to_char(SYSDATE,''DD/MM/YYYY'') '+
                            ' and TI.RESULTAD IN (''R'',''C'')  and td.turnoid='+inttostr(CODTURNOID) +
                            ' and  ti.inspfina=''S''  and td.dvdomino='+#39+trim(PatenteNueva.Patente)+#39+
                            ' and ti.codinspe=td.codinspe '+
                           ' and TI.CODVEHIC =TD.CODVEHIC') ;
                    Open;
                    while not eof do
                    begin
                     mensajerevews.show;
