              ...
              FrmFinal:=TFrmFinal.CreateFromInspeccion(FInspecciones);
                try
                  ...
                  FrmFinal.Label4.Caption:='';
                   {tipo combustible}
                    FrmFinal.Label8.Caption:='';
                      With TSQLQuery.Create(nil) do
                      try
                       Close;
                       SQL.Clear;
                       SQLConnection:=mybd;
                       SQL.Add('select tipogas from tvehiculos  where patenten=:patente');
                       Params[0].Value:=ValueByName[FIELD_MATRICUL];

                       ExecSQL;
                       open;
                         if trim(fieldbyname('tipogas').asstring)='G' THEN
                            FrmFinal.Label8.Caption:='GNC';

                         if trim(fieldbyname('tipogas').asstring)='N' THEN
                         begin
                            FrmFinal.Label8.Caption:='NAFTA';
                            FrmFinal.Pkme.Color :=clSilver;
                         end;
                         if trim(fieldbyname('tipogas').asstring)='L' THEN
                         begin
                            FrmFinal.Label8.Caption:='GASOIL';
                            FrmFinal.Pco2.Color := clSilver;
                         end;
                         if trim(fieldbyname('tipogas').asstring)='M' THEN
                            FrmFinal.Label8.Caption:='MEZCLA';
                  ...
                  FrmFinal.Caption:='FINALIZAR INSPECCIÓN: '+TRIM(ValueByName[FIELD_CODINSPE]); 
          ...
          if MessageDlg('PLANTA DE VERIFICACION',sMensaje,mtInformation,[mbIgnore, mbCancel],mbIgnore,0) = mrIgnore then
            begin
              if Not Finspecciones.Cancelar then
                ...
                begin
                    {informar turno como ausente}
                    
                         aQ:=TsqlQuery.Create(nil);
                         With aQ do
                          Try
                            SQLConnection:=FDataBase;
                            Sql.Add(Format('UPDATE TDATOSTURNO SET AUSENTE=''S'', REVISO=''N'' WHERE CODINSPE= ''%S'' AND ANIO=''%S'' ',[ValueByName[FIELD_CODINSPE],ValueByName[FIELD_EJERCICI]]));
                            ExecSql;
                          Finally
                          Close;
                          Free;
                     ...
                     if Application.MessageBox( 'Usted ha cancelado una inspección. ¿ Desea informar este turno como ausente a SUVTV ?', 'Informar Ausente a SUVTV',
                          MB_ICONQUESTION OR MB_YESNO ) = ID_YES then
                      begin
                           aQ:=TsqlQuery.Create(nil);
                         With aQ do
                          Try
                            SQLConnection:=FDataBase;
                            Sql.Add(Format('select turnoid  from tdatosturno WHERE CODINSPE= ''%S'' AND ANIO=''%S'' ',[ValueByName[FIELD_CODINSPE],ValueByName[FIELD_EJERCICI]]));
                            ExecSql;
                            open;
                         ...
                         frmturnos.informar_ausentes_turnoid(turnoid);
...
procedure TFrmLinea.Desbloquear1Click(Sender: TObject);
var  aQ: TsqlQuery;
     matricula:string;
begin

 matricula:=trim(DBGTESTADOINSPB.Fields[1].asstring);
if Application.MessageBox( pchar('¿Desea desbloquear '+matricula+'?'), 'Desbloquear',
  MB_ICONQUESTION OR MB_YESNO ) = ID_YES then
  begin
...
With aQ do
  Try
    SQLConnection:=FDataBase;
    Sql.Add(Format('update TESTADOINSP set usuario=null WHERE matricul = ''%S'' ',[matricula]));
    ExecSql;
  Finally
    Close;
    Free;
