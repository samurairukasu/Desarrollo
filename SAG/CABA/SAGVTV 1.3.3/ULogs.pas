...
//    then hf := -1                //MODI MIGRA
    then hf := INVALID_HANDLE_VALUE
    else hf := i
end;
...
begin
    ...
    then begin
        CloseHandle(hf);
//        hf := -1;
        hf := INVALID_HANDLE_VALUE; //MODI MIGRA
    end;
...
begin
    ...
    then begin
        ...
        then begin
            Literal := s + ^M + ^J;
            if (not WriteFile(hf,PChar(Literal)^,Length(Literal),pBytesW,nil)) or (Length(Literal) <> pBytesW)
...
begin
     IF c IS TForm
        THEN WITH (c AS TForm) DO
             ...
             END
        ELSE IF c IS TClientDataSet
                THEN WITH (c AS TClientDataSet) DO
                     BEGIN
                          PonLineaComponente(nivel,orden,'ClientDataSet '+ComponenteStr(c));
                          IF commandtext <> ''
                             THEN 
                                  BEGIN
                                       PonLineaComponente(nivel,-1,'- Sentencia SQL:');
                                       PonLineaComponente(nivel+1,i,commandtext);
                                  END;
                          IF ParamCount>0
                             THEN WITH Params DO
                                  BEGIN
                                  ...
                                  END
                     END
        ELSE IF c IS TsqlQuery
                THEN WITH (c AS TsqlQuery) DO
                     BEGIN
