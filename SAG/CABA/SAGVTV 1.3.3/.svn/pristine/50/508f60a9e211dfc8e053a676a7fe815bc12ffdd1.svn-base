...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, Exportar2PDF,
  StdCtrls, Db,SqlExpr, Buttons, globals, usagClasses, ucdialgs, ExtCtrls, shellapi,
  uSagEstacion;
...
function TfmDatosInsp.genera_pdf:boolean;
var  Gpdf:TExportar2PDFSyn;
begin
   Gpdf:=TExportar2PDFSyn.create;
   Gpdf.rutaPDF:=ExtractFilePath(application.Name)+'temp_informe.pdf';

   Gpdf.exportar2PDF(frmMedAuto.repmedauto);
   Gpdf.Free;
        ...
        with tfrmMedAuto.Create(application) do
         try
           ...
           //pdf
             ...
             Gpdf:=TExportar2PDFSyn.create;
             Gpdf.rutaPDF:=ExtractFilePath(application.Name)+'temp_informe.pdf';
             Gpdf.exportar2PDF(repmedauto);
             Gpdf.Free;




              ShellExecute(Handle,nil,PChar(ExtractFilePath(application.Name)+'temp_informe.pdf'),'','',SW_SHOWNORMAL);
           //
