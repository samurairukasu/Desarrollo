...
begin
    try
        ...
        try
            aQ.SQLConnection := Self.Database;
         // martin, esto es para provinvia, se reemplaza para caba la linea de abajo.
        //  aQ.SQL.Add (Format('SELECT GETIMPONETO(%d,%d,%d) FROM DUAL',[aEjercicio,aCodigo,aCliente_ID]));

          aQ.SQL.Add (Format('SELECT Getimponeto_caba(%d,%d,%d) FROM DUAL',[aEjercicio,aCodigo,aCliente_ID]));
    ...
    begin
        ...
        Try
            ...
            then
             ...
             If ValueByName[FIELD_CREDITCL]='X'
            then
               Result:=tTipoCredito(2)
              ELSE
...
cantInspe:=aInspections.RecordCount;
    ...
    with TSQLQuery.create(application) do
      try
        ...
        For I:=1 to High(TRevisor) do
          Begin
            SQL.Clear;
            SQL.Add('SELECT (R.APPEREVIS ||'' ''|| R.NOMREVIS) FROM TREVISOR R, TEMP_DATINSPECC I WHERE I.EJERCICI = :EJERCICIO AND ');
    ...
    with TSQLQuery.create(application) do
      try
        SQLConnection:=self.database;
          Begin
            SQL.Clear;
            SQL.Add('SELECT I.NUMREVZ1, I.NUMREVZ2, I.NUMREVZ3 FROM TEMP_DATINSPECC I WHERE I.EJERCICI = :EJERCICIO AND I.CODINSPE = :CODINSPE');
            Params[0].Value:= ValueByName[FIELD_EJERCICI];
            Params[1].Value:= Valuebyname[FIELD_CODINSPE];
            Open;
