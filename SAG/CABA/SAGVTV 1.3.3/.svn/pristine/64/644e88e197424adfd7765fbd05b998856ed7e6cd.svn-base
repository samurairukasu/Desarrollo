...
const
    ...
    D_NAC = 2;   C_NAC = 16;
    D_AP1 = 8;   C_AP1 = 16;  C_APA = 17;  C_APB = 18;
...
var
    ...
    anu,inu: integer;  anioAct, anioSig,auxSql: string;
begin
  try
...
(*        try
  ...
  //     excelHoja.cells[(f + D_TVC), C_TVC].value :=Format('%.2f',[StrToFloat(iTtlContSales)]);
   ...
   //    excelHoja.cells[(f + D_NAC), C_NAC].value :=iNAC;
      ...
      with tsqlquery.Create(application) do
       try
        ...
        while not Eof  do
             begin
                 //cantidad
                anioSig:= fields[3].asstring;
                anioAct:=FormatDateTime('yyyy',Date);
                if anioAct <>  anioSig then f:=f+1 else f:=f+8;
                excelHoja.cells[f, 13].value :=fields[0].asstring;
                //oblea menor año actual
                excelHoja.cells[f, 14].value :=fields[1].asstring;
                //oblea mayor año actual
                excelHoja.cells[f, 15].value :=fields[2].asstring;
                //Año-Proveedor
      ...
      excelHoja.cells[(f), 13].value := obleasutiliz ;

       // obleas anuladas
      excelHoja.cells[(f+1), 13].value :=iOblAnula;
       // obleas inutilizadas
      excelHoja.cells[(f+2) , 13].value :=iOblInutil;

    //  excelHoja.cells[(f+2) , 21].value :=inttostr(strtoint(iOblInutil)+strtoint(iOblAnula)+strtoint(obleasutiliz));
      excelHoja.cells[(f+3) , 13].value :=strtoint(iOblInutil)+strtoint(iOblAnula)+obleasutiliz;

     {*
       // Aprobados
       excelHoja.cells[(f-9), 13].value :=iNACR-Observaciones-iNR;

       // Observaciones
       excelHoja.cells[(f-8), C_OBS].value :=Observaciones;

       // rechazados
       excelHoja.cells[(f-7), 13].value :=iNR;

       // total aptos, condicionales, rechazados
       excelHoja.cells[(f-6), 13].value :=iNACR;
      ...
      *}
      ...
      with tsqlquery.Create(application) do
       try
        SQLConnection := mybd;
        sql.clear;
        auxsql:= 'SELECT COUNT (*) FROM TINSPECCION i inner join Tdatosturno t on i.codinspe=t.codinspe '+
                 ' WHERE i.fechalta between TO_DATE('''+DateIni+''', ''DD/MM/YYYY HH24:MI:SS'') and  '+
                 ' TO_DATE('''+DateFin+''', ''DD/MM/YYYY HH24:MI:SS'') AND inspfina = ''S'' AND i.tipo=''A'''+
                 ' AND codfactu IS NULL  AND facturado not in(''O'') and tipoturno=''EX''';
        sql.Add(auxsql);
        Open;
        First;
        while not Eof  do
        begin
            excelHoja.cells[30, 6].value :=fields[0].asstring;
            Next;
        end ;
       finally
        free;
      ...
      with tsqlquery.Create(application) do
       try
        ...
        auxsql:= 'SELECT COUNT (*) FROM TINSPECCION i inner join Tdatosturno t on i.codinspe=t.codinspe '+
                 ' WHERE i.fechalta between TO_DATE('''+DateIni+''', ''DD/MM/YYYY HH24:MI:SS'') and  '+
                 ' TO_DATE('''+DateFin+''', ''DD/MM/YYYY HH24:MI:SS'') AND inspfina = ''S'' AND i.tipo=''E'''+
                 ' AND codfactu IS NULL  AND facturado not in(''O'') and tipoturno=''EX''';
        sql.Add(auxsql);
        Open;
        First;
        while not Eof  do
        begin
            excelHoja.cells[31, 6].value :=fields[0].asstring;
            Next;
        end ;
       finally
        free;
      ...
      with tsqlquery.Create(application) do
       try
        ...
        auxsql:= 'SELECT COUNT (*) FROM TINSPECCION i inner join Tdatosturno t on i.codinspe=t.codinspe '+
                 ' WHERE i.fechalta between TO_DATE('''+DateIni+''', ''DD/MM/YYYY HH24:MI:SS'') and  '+
                 ' TO_DATE('''+DateFin+''', ''DD/MM/YYYY HH24:MI:SS'') AND inspfina = ''S''  AND codfactu IS NULL  AND facturado=''O''';
        sql.Add(auxsql);
        Open;
        First;
        while not Eof  do
        begin
            excelHoja.cells[32, 6].value :=fields[0].asstring;
            Next;
        end ;
       finally
        free;
