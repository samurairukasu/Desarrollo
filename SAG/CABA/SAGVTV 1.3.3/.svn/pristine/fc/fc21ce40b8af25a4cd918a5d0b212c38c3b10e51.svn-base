...
closefile(archi);
    ...
    begin
    INFORME:='NO';
       respuesta:='Atención. La inspección no será informada por: '+respuesta+'. Se informará dentro de los 10 minutos automáticamente.';
...
closefile(archi);
    ...
    IF TRIM(codmensaje)='1' THEN
       ...
       Application.MessageBox( 'Se ha procesado correctamente.', 'Atención',
       MB_ICONINFORMATION );
        EXCEPT
         MyBD.Rollback(TD);
          Application.MessageBox( 'Se produjo un error la intentar procesar el cambio en el vehiculo.','Atención', MB_ICONSTOP );
...
begin
 ...
 SQL.Add('SELECT COUNT(*) FROM TDATOSTURNO WHERE FECHATURNO=TO_date('+#39+trim(fechahoy)+#39+',''dd/mm/yyyy'') and ausente in (''N'') and reviso=''S'' and modo=''P'' and estadoid in (1,3,7,4) ');
  ExecSQL ;
  OPEN;
  label3.Caption:='REVISÓ: '+FIELDS[0].ASSTRING;
  finally
  close;
  free;
  end;
     ...
     SELF.RxMemoryData1modelo.Value:=TRIM(FIELDBYNAME('dvmodelo').ASSTRING);
      ...
      if TRIM(FIELDBYNAME('tipoinspe').ASSTRING)='R' then
       ...
       begin
           ...
           self.RxMemoryData1es.Value:='Periódica';
...
TdbGrid(sender).DefaultDrawColumnCell(Rect, Datacol, Column, State);
 ...
 if (Datacol = 0)  then
begin
   if TRIM(self.RxMemoryData1es.Value)='Periódica' THEN
   begin
