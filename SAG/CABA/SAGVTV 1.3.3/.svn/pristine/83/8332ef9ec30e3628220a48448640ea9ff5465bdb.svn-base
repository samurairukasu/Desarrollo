...
interface
    ...
    //True si se desea reimprimir una factura y False si es una Nota de Crédito.
...
//    procedure InspeccionesDiferidas;
    ...
    function reimprimir_informe_caba:boolean;
implementation


uses
   UREIMPNC,  Uuingresapatentecabacert, fureimprimecertcaba,
   UCOMPNF,
   UCDIALGS, ULOGS, Forms, Controls, SysUtils,
   USAGCLASSES,
   Globals,
   USAGESTACION,
   UREIMPCER,
...
function reimprimir_informe_caba:boolean;
var frmcert:Tuingresapatentecabacert;
PATENTE:STRING;
LISTADOINFOR:Tureimprimecertcaba;
IFECHAALTA:STRING;
IFECHAVENCI:STRING;
INUMOBLEA,NRO_INFORME, INFORME1,INFORME2:STRING;
IRESULTADO,PROPIETARIO,CONDUCTOR:STRING;
ICODPRO:LONGINT;
ICODCON,ICODINSPE,IANIO,CANT:LONGINT;
aContexto,ACLIENTES,AINFORME:TSQLQuery;
begin
PATENTE:='';
IFECHAALTA:='';
IFECHAVENCI:='';
...
CONDUCTOR:='';
      ...
      frmcert := Tuingresapatentecabacert.Create(nil);
       try
          if (frmcert.ShowModal = mrOk) then
          begin
           PATENTE:=TRIM(FRMCERT.Edit1.Text);


           LISTADOINFOR:=Tureimprimecertcaba.CREATE(NIL);
            TRY
                LISTADOINFOR.Label1.Caption:='PATENTE: '+TRIM(PATENTE);
                aContexto:=TSQLQuery.Create(nil);
                with aContexto do
                 begin
                   SQLConnection:=mybd;
                   sql.add(' select I.FECHALTA AS IFECHAALTA, I.FECVENCI AS IFECHAVENCI, '+
                   ' I.NUMOBLEA AS INUMOBLEA,I.RESULTAD AS IRESULTADO,I.CODCLPRO AS ICODPRO,I.CODCLCON AS ICODCON,'+
                   'I.CODINSPE AS ICODINSPE, I.EJERCICI AS IEJERCICI '+
                   ' from TINSPECCION I,TVEHICULOS V    '+
                           ' WHERE I.CODVEHIC=V.CODVEHIC  and i.inspfina=''S'' '+
                           '  AND V.PATENTEN=:UPATENTE order by  i.codinspe desc');
                   ParamByName('UPATENTE').Value:=PATENTE;
                   try
                    Open;
                      LISTADOINFOR.RxMemoryData1.Close;
                       LISTADOINFOR.RxMemoryData1.Open;
                    WHILE NOT EOF DO
                    BEGIN
                     IFECHAALTA:=trim(copy(TRIM(FIELDBYNAME('IFECHAALTA').ASSTRING),1,11));
                     IFECHAVENCI:=TRIM(FIELDBYNAME('IFECHAVENCI').ASSTRING);
                     INUMOBLEA:=TRIM(FIELDBYNAME('INUMOBLEA').ASSTRING);
                     IRESULTADO:=TRIM(FIELDBYNAME('IRESULTADO').ASSTRING);
                     ICODPRO:=FIELDBYNAME('ICODPRO').ASINTEGER;
                     ICODCON:=FIELDBYNAME('ICODCON').ASINTEGER;
                     ICODINSPE:=FIELDBYNAME('ICODINSPE').ASINTEGER;
                     IANIO:=FIELDBYNAME('IEJERCICI').ASINTEGER;

                           ACLIENTES:=TSQLQuery.Create(nil);
                           ACLIENTES.SQLConnection:=mybd;
                           ACLIENTES.sql.add(' SELECT NOMBRE,APELLID1,APELLID2 FROM TCLIENTES WHERE CODCLIEN=:ICODPRO');
                           ACLIENTES.ParamByName('ICODPRO').Value:=ICODPRO;
                           ACLIENTES.Open;
                           PROPIETARIO:=TRIM(ACLIENTES.FIELDBYNAME('NOMBRE').ASSTRING)+' '+TRIM(ACLIENTES.FIELDBYNAME('APELLID1').ASSTRING);
                           ACLIENTES.Close;
                           ACLIENTES.Free;
                             ...
                             ACLIENTES:=TSQLQuery.Create(nil);
                           ACLIENTES.SQLConnection:=mybd;
                           ACLIENTES.sql.add(' SELECT NOMBRE,APELLID1,APELLID2 FROM TCLIENTES WHERE CODCLIEN=:ICODCON');
                           ACLIENTES.ParamByName('ICODCON').Value:=ICODCON;
                           ACLIENTES.Open;
                           CONDUCTOR:=TRIM(ACLIENTES.FIELDBYNAME('NOMBRE').ASSTRING)+' '+TRIM(ACLIENTES.FIELDBYNAME('APELLID1').ASSTRING);
                           ACLIENTES.Close;
                           ACLIENTES.Free;



                       LISTADOINFOR.RxMemoryData1.Append;

                       LISTADOINFOR.RxMemoryData1FECHA.Value:=TRIM(IFECHAALTA);

                       LISTADOINFOR.RxMemoryData1RESULTADO.Value:=TRIM(IRESULTADO);
                       LISTADOINFOR.RxMemoryData1OBLEA.Value:=TRIM(INUMOBLEA);

                          AINFORME:=TSQLQuery.Create(nil);
                           AINFORME.SQLConnection:=mybd;
                           AINFORME.sql.add(' SELECT NROCERTIFICADO,NRO_INFORME FROM CERTIFICADOINSPECCION WHERE CODINSPE=:CODINSPE');
                           AINFORME.ParamByName('CODINSPE').Value:=ICODINSPE;
                           AINFORME.Open;
                           CANT:=1;
                           WHILE NOT AINFORME.EOF DO
                            BEGIN
                                 IF CANT=1 THEN
                                  BEGIN
                                   INFORME1:=TRIM(AINFORME.FIELDBYNAME('NROCERTIFICADO').ASSTRING);
                                   NRO_INFORME:=TRIM(AINFORME.FIELDBYNAME('NRO_INFORME').ASSTRING);
                                  END;

                                 IF CANT=2 THEN
                                  BEGIN
                                   INFORME2:=TRIM(AINFORME.FIELDBYNAME('NROCERTIFICADO').ASSTRING);
                       ...
                       LISTADOINFOR.RxMemoryData1INFORME1.Value:=TRIM(INFORME1);
                       LISTADOINFOR.RxMemoryData1INFORME2.Value:=TRIM(INFORME2);
                       LISTADOINFOR.RxMemoryData1NROINSPECCION.Value:=TRIM(NRO_INFORME);



                       LISTADOINFOR.RxMemoryData1CODINSPE.VALUE:=ICODINSPE;
                       LISTADOINFOR.RxMemoryData1ANIO.VALUE:=IANIO;
                       LISTADOINFOR.RxMemoryData1TITULAR.VALUE:=TRIM(PROPIETARIO);
                       LISTADOINFOR.RxMemoryData1CONDUCTOR.Value:=TRIM(CONDUCTOR);
                       LISTADOINFOR.RxMemoryData1VENCE.VALUE:=TRIM(IFECHAVENCI);
                       LISTADOINFOR.RxMemoryData1.POST;
