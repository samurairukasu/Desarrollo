...
interface
    ...
    BCancelar: TBitBtn;
    BBOtros: TBitBtn;
    BBInsertar_Localidad_CP: TBitBtn;
    ...
    procedure BBOtrosKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure BBOtrosMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure BBOtrosClick(Sender: TObject);
    procedure RBexentoClick(Sender: TObject);
    procedure BBInsertar_Localidad_CPClick(Sender: TObject);
...
//                fLocalidad.Open;
                ...
                //CETDocu.Text := S_TIPO_DOCUMENTO[Documento];
                 ...
                 if trim(UnCliente.ValueByName[FIELD_CREDITCL])='X' then
                    BEGIN
                      SELF.RBexento.Checked:=TRUE;
                      RBexentoClick(RBexento);
                     END ELSE BEGIN

                            if trim(UnCliente.ValueByName[FIELD_CREDITCL])='S' then
                               BEGIN
                                 SELF.RBDeCredito.Checked:=TRUE;
                                 RBDeCreditoClick(RBDeCredito);
                                END ELSE
                                     BEGIN
                                         if trim(UnCliente.ValueByName[FIELD_CREDITCL])='N' then
                                          BEGIN
                                            SELF.RBDePago.Checked:=TRUE;
    ...
    begin
        ...
        if UnCliente.Credito in [ttcCredito]
        then begin
            RBDeCredito.Checked := TRUE;
            RBDeCreditoClick(RBDeCredito)
        end;

         if UnCliente.Credito in [ttcexento]
        then begin
           self.RBexento.Checked:=true;
            RBexentoClick(RBexento);
        end ;
...
begin
    //Aceptar Datos
    Try
      if TRIM(CEDireccion.Text)='' then
    begin

             showmessage('ATENCION.', 'Debe ingresar el domicilio.');
        ...
        if TRIM(ceNumero.Text)='' then
    begin

             showmessage('ATENCION.', 'Debe ingresar el nro del  domicilio.');
           ceNumero.SetFocus;
           ...
           if TRIM(CBProvincia.Text)='' then
    begin

             showmessage('ATENCION.', 'Debe seleccionar la provincia.');
           ...
           if TRIM(CBCodposta.Text)='' then
    begin

             showmessage('ATENCION.', 'Debe seleccionar el codigo postal.');
           CBCodposta.SetFocus;
            ...
            if TRIM(CBLocalidad.Text)='' then
    begin

             showmessage('ATENCION.', 'Debe seleccionar la localidad.');
...
procedure TFAMCliente.RBexentoClick(Sender: TObject);
begin
  If fUpdating Then Exit;
        fUpdating:=True;
        Try
            if (Sender as TRadioButton).Checked
            then begin
                UnCliente.ValueByName[FIELD_CREDITCL]:='X';
                PCredito.Caption := ' ' + S_TIPO_CREDITO[ttcexento];
                PCredito.Font.Color := clred;
                ICredito.Picture := ISinCredito.Picture;
                IMoroso.PIcture := ICorriente.Picture;
                RLSinCredito.Visible := FALSE;
            end;
...
procedure TFAMCliente.BBInsertar_Localidad_CPClick(Sender: TObject);
begin
    UFInserta_Localidad_CP.showmodal;
end;
