...
function devolverfecha(strin: string):tdatetime ;
...
uses SysUtils, {$IFDEF WIN32} Windows, {$ENDIF} RTLConsts, StrUtils, rxStrUtils;
...
begin
  ...
  insert(':',strin,12);
  insert(':',strin,15);
  fecha:=strtodatetime(FormatDateTime ('YY/MM/DD HH:MM:SS',StrToDateTime(strin)));
  result:=fecha;
end;
