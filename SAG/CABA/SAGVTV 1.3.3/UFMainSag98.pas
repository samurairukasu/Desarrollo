...
interface
    ...
    uses
        ...
        ExcelXP,
        OleServer,
        WinSkinData,    Unit1descargar_turno_suvtv,
        StdCtrls,
        ComCtrls,
        SpeedBar,
        Ufrminformacionpago,
        ExtCtrls,
        Classes,
        Unitfrmregistrar_remitos_certificados,
        Unitfrmver_remitos_certificados,
        Graphics,
        UGENERA_NC_ELECTRONICA,
        UnitfrmsMOTIVO_CANCELAMIENTO,
        Unit_borrar_defecto_patente,
        Controls,
        Ufrconfiguracion_impresoras,
        class_impresion,
        UcertificadoCABA,
        Forms,
        Menus,
        ...
        DB,
        SqlExpr,
        StrUtils,   Ucambiodocumentotitular,
        Unborrar_defectos_solo_reveApta;
    ...
    Certificados2: TMenuItem;
    Imprimir2: TMenuItem;
    Anuladas3: TMenuItem;
    Registrar2: TMenuItem;
    N23: TMenuItem;
    RegistrosdeCertificadosEntregados1: TMenuItem;
    Registrar3: TMenuItem;
    Ver1: TMenuItem;
    N24: TMenuItem;
    EliminarDefecto1: TMenuItem;
    PorPatente1: TMenuItem;
    PorCadenadeDefecto1: TMenuItem;
    Conciliacion1: TMenuItem;
    ListadoPagos: TMenuItem;
    otalesDiarios1: TMenuItem;
    N25: TMenuItem;
    InformacionPagos1: TMenuItem;
    ListadoMediciones1: TMenuItem;
    N26: TMenuItem;
    SolicitarCambiodeDominioenSUVTV1: TMenuItem;
    N27: TMenuItem;
    CambiarDocumentoaTitular1: TMenuItem;
    ServiciosPrestados: TMenuItem;
    MercadoPago1: TMenuItem;
    Epago1: TMenuItem;
    ResumenDiario1: TMenuItem;
    ServiciosPrestadosDetallado1: TMenuItem;
    DigitoVerificador: TMenuItem;
    ...
    procedure PorCadenadeDefecto1Click(Sender: TObject);
    procedure PorPatente1Click(Sender: TObject);
    procedure otalesDiarios1Click(Sender: TObject);
    procedure InformacionPagos1Click(Sender: TObject);
    procedure SolicitarCambiodeDominioenSUVTV1Click(Sender: TObject);
    procedure CambiarDocumentoaTitular1Click(Sender: TObject);
    procedure ServiciosPrestadosClick(Sender: TObject);
    procedure MercadoPago1Click(Sender: TObject);
    procedure Epago1Click(Sender: TObject);
    procedure ResumenDiario1Click(Sender: TObject);
    procedure DigitoVerificadorClick(Sender: TObject);
...
{$R *.DFM}
    uses
        ...
        UFSERVICIOSPRESTADOS,  GenerateResumenDiarioADM, UFDIGITO_VERIFICADOR,
        UFLISTADOPAGOS_MP,
        UFLISTADOPAGOS_EP,
        UCOMUNIC,
        UCOMUNICGNC,
        ...
        UAltasdeOblea,
        UAltadeCertificados, UFListadoMediciones,
        UReImpresionObleas,
        UFRestaurarOblea,
        ufPrintCertAn,
        uSQLExec,
        ...
        Ulistadocierrez,
        ufInformeRevesExternasToPrint,
        UFRESULTADINSPECTIEMPOS,
        Ufaletas_vehiculos,
        Unitsincronizacion_cf,
        Uncambiar_calificacion_defectos_solo_reve,
        Unifinaliza,
        Unitpide_fecha,
        Unitfrmdiseniofactelectronica,
        UniCONTROLSERVICIO_SVVTV,
        UFCertificadosAnulados,
        UFConciliacionPorTurnos,
        Unit1reimprimir_factura;
...
Function TFMainSag98.Genera_informacion_pagos:boolean;
var aq:TSQLQuery;
begin
       with Tfrminformacionpago.Create(Application) do
        try
            try
                If not GetDates(DateIni,DateFin) then Exit;

                 FTmp.Temporizar(TRUE,FALSE,'Listado de Dominios y Nro de Comprobantes', 'Generando el informe listado de dominios y nro de comprobantes.');
                 Application.ProcessMessages;

    with TSQLStoredProc.Create(application) do
    try
        SQLConnection := MyBD;
        StoredProcName := 'PQ_INFORMACIONPAGOS.INFORMACIONPAGOS';
        Prepared := true;
        ...
        Free;
          ...
          aq.SQL.Add('select * from TTMP_INFORMACIONPAGO');
          aq.ExecSQL;
          aq.open;
         RxMemoryData1.Close;
         RxMemoryData1.Open;
         ...
         begin
              ...
              RxMemoryData1fecha.Value:=trim(aq.fieldbyname('FECHALTA').AsString);
              RxMemoryData1cliente.Value:=trim(aq.fieldbyname('Cliente').AsString);
              RxMemoryData1nrocomprobante.Value:=trim(aq.fieldbyname('NRO_COMPROBANTE').AsString);
              RxMemoryData1dominio.Value:=trim(aq.fieldbyname('DOMINIO').AsString);
              RxMemoryData1idpago.Value:=aq.fieldbyname('IDPAGO').Asinteger;
              RxMemoryData1tipocomprobanteafip.Value:=trim(aq.fieldbyname('TIPOCOMPROBANTEAFIP').AsString);
              RxMemoryData1importetotal.Value:=aq.fieldbyname('IMPORTETOTAL').Asinteger;
...
If bActivandose then
  ...
  InitError('Ya existe una copia del programa en ejecución')
    ...
    Caption := 'Planta Verificadora de Vehículos';
...
procedure TFMainSag98.BorrarArchivos(Ruta: string);
var
  SR: TSearchRec;
begin
        ...
        if AccederAServicio(aService,ApplicationUser,PasswordUser, GestorSeg) then
          begin
             case aService of
               ...
               ID_INFORMACION_PAGO_ARQUEOS:
               begin
                   Genera_informacion_pagos;
               end;
              ...
              ID_SERVICIO_ARQUEO_CAJA_EXTENDIDO_CONTADO:
               ...
               ID_SERVICIO_RESUMEN_DIARIO:
               begin
                  GenerateResumenDiario;
               end;

               ID_SERVICIO_SERVICIOS_PRESTADOS:
               begin
                  GenerateServiciosPrestados;
               end;

               ID_SERVICIO_DIGITO_VERIFICADOR:
               begin
                  Digito_Verificador;
               end;

               ID_SERVICIO_LISTADO_MERCADOPAGO:
               begin
                  GenerateListadoMercadoPago;
               end;

               ID_SERVICIO_LISTADO_EPAGO:
               begin
                  GenerateListadoEpago;
               end;
              ...
              ID_SERVICIO_REGISTRO_OBLEAS_ANUL:
               ...
               223:
               begin
                  GenerateConciliacion;
               end;
               225:
               begin
                  Generatetotales_diarios;
               end;
               226:
               begin
                  GenerateListadoMediciones;
               end;
...
frmturnos.idusuariologeado:= GLOBALS.ID_USUARIO_LOGEO_SAG;
frmturnos.showmodal;
...
with Tfrconfiguracion_impresoras.Create(Nil) do
 ...
 FUNCTION TFMainSag98.MOTIVO_CANCELAMIENTO:STRING;
 BEGIN
 ...
 procedure TFMainSag98.crear_registro_sql;
 var llave:string;
 Registro:TRegistry;
begin  
llave:='\SOFTWARE\Wow6432Node\'+COMPANY_NAME+'\'+APPLICATION_NAME+'\BD\'+Alias_SQL;
If Registro.KeyExists('HKEY_LOCAL_MACHINE'+llave) then
...
StoredProc: TSQLStoredProc;    aq1:TSQLQuery;
 aq,aqb:TSQLQuery;  idturno,codinspe,TURNOID,CODVEHIC,EJERCICI,turnoidpadre:longint;

  i,autorizo: Integer;
  si,num,motivo_cancelar_certif: String;
  Hoja: _WorkSheet;
   aqcancel,aqrelacion:TSQLQuery;
    TIMPRE:TIMPRESIONCABA;
 begin
 TIMPRE:=TIMPRESIONCABA.Create;
 TIMPRE.imprimir_certificado_caba(15302,2016,0,false,'x');
 TIMPRE.Free;
end;
...
procedure TFMainSag98.ReimprimirFacturas1Click(Sender: TObject);
begin
reimprimir_factura.showmodal;
end;

procedure TFMainSag98.Registrar3Click(Sender: TObject);
begin
with Tfrmregistrar_remitos_certificados.Create(Nil) do
   try
    showmodal;
     finally
      free;
...
procedure TFMainSag98.Ver1Click(Sender: TObject);
begin
with Tfrmver_remitos_certificados.Create(Nil) do
   try
    showmodal;
     finally
      free;
...
procedure TFMainSag98.PorCadenadeDefecto1Click(Sender: TObject);
begin
with Tborrar_defectos_solo_reveApta.Create(Nil) do
   try
    showmodal;
...
begin
with Tborrar_defecto_patente.Create(Nil) do
   try
    showmodal;
     finally
      free;
...
procedure TFMainSag98.otalesDiarios1Click(Sender: TObject);
var
aServicio : integer;
begin
aServicio := (Sender as TComponent).Tag;
...
procedure TFMainSag98.InformacionPagos1Click(Sender: TObject);
var
aServicio : integer;
begin
aServicio := (Sender as TComponent).Tag;
...
procedure TFMainSag98.ServiciosPrestadosClick(Sender: TObject);
var
aServicio : integer;
begin
aServicio := (Sender as TComponent).Tag;
...
procedure TFMainSag98.MercadoPago1Click(Sender: TObject);
var
aServicio : integer;
begin
aServicio := (Sender as TComponent).Tag;
...
procedure TFMainSag98.Epago1Click(Sender: TObject);
var
aServicio : integer;
begin
aServicio := (Sender as TComponent).Tag;
...
procedure TFMainSag98.ResumenDiario1Click(Sender: TObject);
var
aServicio : integer;
begin
aServicio := (Sender as TComponent).Tag;
...
procedure TFMainSag98.DigitoVerificadorClick(Sender: TObject);
var
aServicio : integer;
begin
aServicio := (Sender as TComponent).Tag;
...
procedure TFMainSag98.SolicitarCambiodeDominioenSUVTV1Click(
  Sender: TObject);
begin
   
ShellExecute(FMainSag98.Handle, 'open', 'DESCARGAR_POR_PATENTE.exe',
'', '', SW_SHOWNORMAL);
end;

procedure TFMainSag98.CambiarDocumentoaTitular1Click(Sender: TObject);
begin
with Tfrmcambiotitular.Create(Nil) do
   try
       edit1.Clear;
       edit2.clear;
       edit3.clear;
