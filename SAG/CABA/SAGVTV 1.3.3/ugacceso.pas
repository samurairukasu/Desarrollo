...
interface
    ...
    const
       ...
       ID_INFORMACION_PAGO_ARQUEOS=300;
       ...
       ID_SERVICIO_SERVICIOS_PRESTADOS = 400;
       ID_SERVICIO_LISTADO_MERCADOPAGO = 401;
       ID_SERVICIO_LISTADO_EPAGO = 402;
       ID_SERVICIO_RESUMEN_DIARIO = 403;
       ID_SERVICIO_DIGITO_VERIFICADOR = 404;
       ID_SERVICIO_CAMBIA_CALIFICACION_DEFECTO_SOLO_PARA_REVE=200;
       ID_SERVICIO_BORRA_DEFECTO_SOLO_REVES_A=201;
    var
 ...
 //   function PermitidoAccesoCambios (var iIDUsuario: integer; var sClaveUsuario: string; GestorSeg: TGestorSeg): boolean;  //
    ...
    uses
        ...
        UCDIALGS,
        ULOGS,
        PASSWORD, VerificarHuella, VerificarSupervisor, Acceso1,
        UfUsuarioCambios;
...
var
iIntentos,idUsuario: integer;
bCorrecto, Resultado,ingresoUsuario : boolean;
NombreUsuario: String; PasUsuario,PasUsuarioDecod: String[32];
nomSupervisor,pasSupervisor: String; zIntentos : integer;

begin
// WB MARTIN
CONECTADO_A_WEBSERVICES_TURNOS:=FALSE;
...
bCorrecto := False;
  Try
    ...
    ingresoUsuario:=True;
    if huellaActiva=1 then
    begin
    // NO BORRAR EL CODIGO COMENTADO

    ingresoUsuario := false;
    while (bCorrecto=false) and (iIntentos<3)  do
    begin
      iIntentos :=iIntentos+1;
      if GetUsuario(idUsuario,NombreUsuario,sClaveUsuario)=true then
      begin
             iIDUsuario := GestorSeg.ObtenerIDUsuario(NombreUsuario);
             GLOBALS.ID_USUARIO_LOGEO_SAG:= iIDUsuario;
             PasUsuario:=sClaveUsuario;
             DecodificarPassword (idUsuario,PasUsuario,PasUsuarioDecod);
             sClaveUsuario:=PasUsuarioDecod;
             ...
             if Permitir = 'CAMBIO' then  Resultado:=True;
      ...
      Showmessage('Gestión de Acceso', 'La huella introducida es incorrecta');
    end;

    iIntentos := 0;

    while (bCorrecto=false) and (iIntentos<3) and (ingresoUsuario=False) do
    begin
      iIntentos :=iIntentos+1;
      if GetSupervisor(NombreUsuario,sClaveUsuario)=true then
      begin
        ...
        if Resultado then ingresoUsuario:=True else Showmessage('Gestión de Acceso', 'La palabra clave introducida es incorrecta');
      end
      else iIntentos:=3;
    end;

    end;
    if (bCorrecto=false) and (ingresoUsuario=True) then
    begin
      iIntentos := 0;
      repeat
            ...
            begin
             NombreUsuario := EdtNombreUsuario.Text;
              ...
              begin
                  ...
                  if Assigned(fTrazas) then  fTrazas.PonAnotacionFmt (TRAZA_SIEMPRE, 2, FILE_NAME, '%S: accede al programa con clave correcta', [NombreUsuario]);
