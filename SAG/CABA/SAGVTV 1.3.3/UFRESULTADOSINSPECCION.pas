  ...
  try
    ...
    try
      ...
      begin
        ...
        ExcelHoja.Cells[F_TTL,C_TTL+2].value := 'Resumen de Inspección';
        ExcelHoja.Cells[F_STT,C_STT+2].value := Format('Planta nº: %S. (%S - %S)', [PutEstacion,Copy(DateIni,1,10), Copy(DateFin,1,10)]);

        f:= F_INI-2;
        ExcelHoja.Cells[f,2].value := TotAprob;
        ExcelHoja.Cells[f+1,2].value := TotRech;
        ExcelHoja.Cells[f+2,2].value := TotCond;
        ExcelHoja.Cells[f+3,2].value := TotAus;
