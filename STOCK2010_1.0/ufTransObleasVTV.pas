...
type
  TfrmTransObleasVTV = class(TForm)
    btnSalir: TBitBtn;
  ...
  procedure DoTransObleasVTV;

var
  frmTransObleasVTV: TfrmTransObleasVTV;

resourcestring
  FILE_NAME = 'UfTransObleasVTV';
  CAPTION = 'Transferencia de Obleas';

implementation

uses Math, urTransObleasVTV;

{$R *.dfm}

procedure DoTransObleasVTV;
begin
  with TfrmTransObleasVTV.Create(application) do
    try
      showmodal;
    finally
      free;
...
procedure TfrmTransObleasVTV.btnSalirClick(Sender: TObject);
begin
  if MyBD.InTransaction then MyBD.Rollback(td);
  close;
end;

procedure TfrmTransObleasVTV.FormCreate(Sender: TObject);
begin
    fOrigen := nil;
    fMovimiento := nil;
    fPlanta := nil;
...
procedure TfrmTransObleasVTV.FormDestroy(Sender: TObject);
begin
    fOrigen.free;
    fMovimiento.free;
    fPlanta.free;
...
procedure TfrmTransObleasVTV.btnAceptarClick(Sender: TObject);
begin
  try
    ...
    begin
      ...
      begin
        fMovimiento.ValueByName[FIELD_TIPO] := MOV_TIPO_TRANSFERENCIA;
        fMovimiento.Post(true);
        ActualizarObleas;
        MyBD.Commit(td);
        DoRepTransObleasVTV(fMovimiento.ValueByName[FIELD_IDMOVIMIENTO],strtoint(fOrigen.valuebyname[FIELD_IDEMPRESA]));
...
procedure TfrmTransObleasVTV.ActualizarObleas;
begin
...
function TfrmTransObleasVTV.ValidatePost: boolean;
begin
  result := false;
...
procedure TfrmTransObleasVTV.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmTransObleasVTV.edCantidadExit(Sender: TObject);
begin
...
procedure TfrmTransObleasVTV.btnCancelarClick(Sender: TObject);
begin
  fMovimiento.Cancel;
  fMovimiento.Append;
end;

procedure TfrmTransObleasVTV.lcbOrigenCloseUp(Sender: TObject);
begin
  lcbOrigen.Value := fOrigen.ValueByName[lcbOrigen.LookUpField];
end;

procedure TfrmTransObleasVTV.lcbDestinoCloseUp(Sender: TObject);
begin
  lcbDestino.Value := fPlanta.ValueByName[lcbDestino.LookUpField];
end;

procedure TfrmTransObleasVTV.edCantCertifEnter(Sender: TObject);
begin
  fMovimiento.ValueByName[FIELD_CANTCERTIF] := edCantidad.Text;
end;

procedure TfrmTransObleasVTV.edCantBolsiEnter(Sender: TObject);
begin
  fMovimiento.ValueByName[FIELD_CANTBOLSI] := edCantidad.Text;
end;

function TfrmTransObleasVTV.CantidadIgualObleas: boolean;
begin
  result := false;
  with tSqlquery.Create(nil) do
    try
