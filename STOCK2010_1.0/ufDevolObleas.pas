...
type
  ...
  procedure DoDevObleasGNC;

var
  frmDevObleasGNC: TfrmDevObleasGNC;

resourcestring
  FILE_NAME = 'UfDevolObleasGNC';
  CAPTION = 'Devolución de Obleas a Depósito';
...
procedure DoDevObleasGNC;
begin
  with TfrmDevObleasGNC.Create(application) do
    try
      showmodal;
    finally
      free;
...
procedure TfrmDevObleasGNC.btnSalirClick(Sender: TObject);
begin
  if MyBD.InTransaction then MyBD.Rollback(td);
  close;
end;

procedure TfrmDevObleasGNC.FormCreate(Sender: TObject);
begin
    fOrigen := nil;
    fMovimiento := nil;
    fPlanta := nil;
    ...
    fOrigen := tPlantas.CreatePlantas(MYBD);
...
procedure TfrmDevObleasGNC.FormDestroy(Sender: TObject);
begin
    fOrigen.free;
    fMovimiento.free;
    fPlanta.free;
...
procedure TfrmDevObleasGNC.btnAceptarClick(Sender: TObject);
begin
  try
...
procedure TfrmDevObleasGNC.ActualizarObleas;
//var i:integer;
begin
...
function TfrmDevObleasGNC.ValidatePost: boolean;
begin
  result := false;
...
procedure TfrmDevObleasGNC.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmDevObleasGNC.edCantidadExit(Sender: TObject);
begin
...
procedure TfrmDevObleasGNC.btnCancelarClick(Sender: TObject);
begin
  fMovimiento.Cancel;
  fMovimiento.Append;
end;

procedure TfrmDevObleasGNC.lcbOrigenCloseUp(Sender: TObject);
begin
  lcbOrigen.Value := fOrigen.ValueByName[lcbOrigen.LookUpField];
end;

procedure TfrmDevObleasGNC.lcbDestinoCloseUp(Sender: TObject);
begin
  lcbDestino.Value := fPlanta.ValueByName[lcbDestino.LookUpField];
end;

procedure TfrmDevObleasGNC.lcbPreparaCloseUp(Sender: TObject);
begin
  lcbPrepara.Value := fPrepara.ValueByName[lcbPrepara.LookupField];
end;

procedure TfrmDevObleasGNC.lcbAutorizaCloseUp(Sender: TObject);
begin
  lcbAutoriza.Value := fAutoriza.ValueByName[lcbAutoriza.LookupField];
end;

procedure TfrmDevObleasGNC.edCanttarjamaEnter(Sender: TObject);
begin
  fMovimiento.ValueByName[FIELD_CANTTARJAMA] := edCantidad.Text;
end;

procedure TfrmDevObleasGNC.edCantetiqgncEnter(Sender: TObject);
begin
  fMovimiento.ValueByName[FIELD_CANTETIQGNC] := edCantidad.Text;
end;

procedure TfrmDevObleasGNC.edCantBolsiEnter(Sender: TObject);
begin
  fMovimiento.ValueByName[FIELD_CANTBOLSI] := edCantidad.Text;
end;

function TfrmDevObleasGNC.CantidadIgualObleas: boolean;
begin
  result := false;
  with tSqlquery.Create(nil) do
    try
