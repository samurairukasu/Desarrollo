...
begin
     ...
     TempKey := 0;
     if not CanCreate or (S = '')
        then Result:=RegOpenKeyEx(GetBaseKey(Relative),PChar(S),0,samDesired,TempKey)=ERROR_SUCCESS
        else Result:=RegCreateKeyEx(GetBaseKey(Relative),PChar(S),0,nil,REG_OPTION_NON_VOLATILE,
                                    samDesired,nil,TempKey,@Disposition)=ERROR_SUCCESS;
     if Result then
        begin
