...
type
  ...
  private
    ...
    FA:tfacturae;
    function guarda_nc(CODFACTU,NUMERO:LONGINT):boolean;
...
{$R *.dfm}
 function TGENERA_NC_ELECTRONICA.guarda_nc(CODFACTU,NUMERO:LONGINT):boolean;
 var aqQm,aqQmf:tsqlquery;
 impresa:string;
 ...
 impresa:='S';
  ...
  aqQm.CLOSE;
  aqQm.SQL.CLEAR;
  aqQm.sql.add('insert into tcontrafac (codcofac,impresa,fechalta,numfactu) '+
  ' values ('+inttostr(secuenciador)+','+#39+TRIM(impresa)+#39+',SYSDATE,'+INTTOSTR(NUMERO)+')');
  aqQm.ExecSQL;
...
RxMemoryData1total.Value:=trim(total);
 ...
 {FIN}
  ...
  nombre_archivo:='Fact'+trim(letra)+''+TRIM(nro_comprobante)+'ORIGINAL.PDF';
  ...
  nombre_archivo:='Fact'+trim(letra)+''+TRIM(nro_comprobante)+'DUPLICADO.PDF';
  ...
  nombre_archivo:='Fact'+trim(letra)+''+TRIM(nro_comprobante)+'TRIPLICADO.PDF';
...
facturabelectronica.RxMemoryData1servicio.Value:='VTV A VEHICULO '+TRIM(patente_para_factura);
...
// facturabelectronica.QRLabel68.Caption:='0.00';
 ...
 aaaa:=copy(trim(fechavence),1,4);
  ...
  BEGIN
    ...
    facturabelectronica.QRLabel26.Caption:=TRIM(aqQm.FIELDBYNAME('cuitfactura').AsString);
  ...
  END;
   ...
   IF TRIM(aqQm.FIELDBYNAME('dfiva').AsString)='C' THEN
    ...
    IF TRIM(AQQm.FIELDBYNAME('dfiva').AsString)='R' THEN
    facturabelectronica.QRLabel29.Caption:='Responsable Inscripto';
   ...
   facturabelectronica.QRLabel31.Caption:=TRIM(aqQm.FIELDBYNAME('dfcalle').AsString)+
   ' '+TRIM(aqQm.FIELDBYNAME('dfnumerocalle').AsString)+' Piso: '+TRIM(aqQm.FIELDBYNAME('dfpiso').AsString)+
   ' Depto: '+TRIM(aqQm.FIELDBYNAME('DFDEPARTAMENTO').AsString)+' - '+TRIM(aqQm.FIELDBYNAME('DFLOCALIDAD').AsString)+
   ' - '+TRIM(aqQm.FIELDBYNAME('DFPROVINCIA').AsString);
...
if self.RadioButton2.Checked=true then
 ...
 aContexto:=TSQLQuery.Create(nil);
  {with aContexto do
   try
    SQLConnection:=mybd;
    sql.add('select PATENTEN from TVEHICULOS where CODVEHIC=:CODVEHIC');
 ...
 {--------------------}
  ...
  if Application.MessageBox( pchar('¿Desea realizar la nota de credito de la factura: '+trim(edit1.Text)+' de '+nombrecliente+' ?'), 'Guardando factura',
...
if fa.procesar_comprobante(id_SERVICIO, tipo_doc, nro_doc, tipo_cbte, punto_vta,
    ...
    fecha_serv_desde, fecha_serv_hasta,moneda_id, moneda_ctz,IMPORTE_PARA21_s,IVA21,fa.Token,fa.Sign,fa.CUIT_EMPRESA,fa.MODO_FAE,'',mybd,1) =true then
begin
NUMEROnc:=FA.ULTIMO;
