...
interface
    ...
    CBTipoMotorMotos: TComboBox;
            ...
            procedure VerificarCliente(docCliente: string;nombre: string;apellido: string; telefono: string);
    ...
    BEGIN
       CODTURNOID:=-1;
        with TSQLQuery.Create(application) do
         try
            SQLConnection:=MYBD;
            SQL.Add('SELECT TURNOID,codvehic FROM TDATOSTURNO  WHERE TO_CHAR(FECHATURNO,''DD/MM/YYYY'') = to_char(SYSDATE,''DD/MM/YYYY'')  and DVDOMINO = '+#39+trim(PATENTE)+#39);
            Open;
            if RecordCount > 0  then
            begin
      ...
      try
        ...
        case aValue of
          ...
          begin
            ...
            if Assigned(fAllInspections) and (fAllInspections.RecordCount > 0) and (fAllInspections.IsReverification(fAllInspections, fvNormal) or es_tursno_reve(CODTURNOID)) then
              begin
    ...
    begin
      ...
      begin
       ...
       try
        ...
        ' contactoemail,dvanio,dvtipo, codclien, cuitfactura from tdatosturno where dvdomino =''%s'' and turnoid=%d',[fPatente.Patente,CODTURNOID]));
        Open;
        if RecordCount > 0  then
        begin
          //MARCA
          ...
          VerificarCliente(Fieldbyname('codclien').AsString,Fields[4].AsString,Fields[5].AsString,Fields[6].AsString);
            ...
            if trim(Fieldbyname('dfnrodocumento').AsString)<>'' then
            begin
               CBTDPropietario.ItemIndex := 0;
               CBTDConductor.ItemIndex := 0;
               CBTDTenedor.ItemIndex := 0;
             //CBTDPropietario.text :='DNI';
               CENroPropietario.text := Fields[3].AsString;
               CENroConductor.text := Fields[3].AsString;
               CENroTenedor.text := Fields[3].AsString;
           end;


             if trim(Fieldbyname('cuitfactura').AsString)<>'' then
              begin
               CBTDPropietario.ItemIndex := 4;
               CBTDConductor.ItemIndex := 4;
               CBTDTenedor.ItemIndex := 4;
             //CBTDPropietario.text :='DNI';
               CENroPropietario.text :=copy(trim(Fieldbyname('cuitfactura').AsString),1,2)+'-'+copy(trim(Fieldbyname('cuitfactura').AsString),3,8)+'-'+copy(trim(Fieldbyname('cuitfactura').AsString),11,1);
               CENroConductor.text :=copy(trim(Fieldbyname('cuitfactura').AsString),1,2)+'-'+copy(trim(Fieldbyname('cuitfactura').AsString),3,8)+'-'+copy(trim(Fieldbyname('cuitfactura').AsString),11,1);
               CENroTenedor.text := copy(trim(Fieldbyname('cuitfactura').AsString),1,2)+'-'+copy(trim(Fieldbyname('cuitfactura').AsString),3,8)+'-'+copy(trim(Fieldbyname('cuitfactura').AsString),11,1);
           end;
...
if assigned(fVehiculos_GNC) then
  begin
    ...
    procedure TFRecepcion.VerificarCliente(docCliente: string;nombre: string;apellido: string; telefono: string);
    var
      aCliente: TClientes;
    begin
                ...
                try
                        //aCliente.Free;
                        aCliente := nil;
                        aCliente := TClientes.CreateFromDataBase(MyBD,DATOS_CLIENTES,'WHERE codclien = '''+docCliente+'''');
                        aCliente.Open;
                        ...
                        begin
                          MostrarDatos_ClienteNuevo (aCliente,docCliente,nombre,apellido,telefono);
                        end;
                finally
                end
    end;
...
if fVerificacion in [fvNormal, fvVoluntaria, fvReverificacion, fvPreverificacionOk, fvVoluntariaReverificacion] then
  begin
  // Si el cliente es de crédito => se puede marcar vehículo oficial
