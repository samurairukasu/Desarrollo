        ...
        case RecordCount of
        {
         LOS UNICOS VALORES POSIBLES SON
          ...
          UNO: begin
            ...
            raise EDuplicadoElRegistro.Create('LA SOLICITUD: ' + IntToStr(SolicitudDelCliente.iHandle) + ' ESTÁ DUPLICADA EN EL SERVIDOR');
        end;
        Close;
        Application.ProcessMessages;
      end;
    ...
    begin
      ...
      if EsPosibleCancelacion (iUnMotivo) then
        ...
        { EL REGISTRO SE HA CANCELADO }
          case iUnMotivo of
          1:begin
              MessageDlg('Notificacion Del Servidor De Impresión',LITERAL_NO_CANCELAR_BLOQUEADO,MtInformation, [mbOk], mbOk,0);
            end;
          2:begin
              MessageDlg('Notificacion Del Serividor De Impresión',LITERAL_NO_CANCELAR_IMPRIMIENDOSE, MtInformation, [mbOk], mbOk,0);
            end;
         end;
...
{ Cuando se cancela la solicitud de impresion, el fin último es que no se imprima
  dicho trabajo.

  Esta operación puede hacerse siempre que el usuario lo desee, por tanto tiene
  prioridad absoluta.
  ...
  Los pasos a seguir siempre son los mismos:
    ...
    Casos en los que esta operacion no puede realizarse:
      ...
      * A : El servidor de impresión ha bloqueado antes el registro porque lo
            está procesando, en este caso dependiendo del estado en que se
            encuentre el cliente se tomará una accion u otra.

      * B : El estado del trabajo es IMPRIMIENDOSE, por tanto ya no se puede
            cancelar, y se deja como está.

      * C : El trabajo ha desaparecido, esto indica que el servidor de impresión
    ...
    begin
      ...
      {LOS UNICOS VALORES POSIBLES SON
        ...
        UNO: case Fields[0].AsInteger of
               4,5 : { Se está Imprimiendo, o a Terminado ya }
...
begin
  ...
  Application.ProcessMessages;
  if MessageDlg( 'Notificacion Del Servidor De Impresión', sMensaje, mtConfirmation, [mbIgnore, mbCancel], mbIgnore, 0) = mrCancel then
