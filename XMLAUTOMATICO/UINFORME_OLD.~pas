...
aCodigoFrecuencia,
aCodigoPropietario,
aCodigoConductor, imprimir_tipo_vehiculo, resultado, FECHA_VENC, oblea : string;
begin
  ...
  if Assigned(FInspeccion) then
    ...
    //INSPECCION
     ...
     // LblXVencimiento.Caption := fInspeccion.ValueByName[FIELD_FECVENCI];
      ...
      begin
        oblea:=fInspeccion.GetNumOblea;
        resultado := 'APTO' ;
        QRLabel35.Caption:='APTO PARA CIRCULAR HASTA '+TRIM(FECHA_VENC);
        QRLabel36.Caption:='';
        // QRLabel39.Caption:=oblea;
        end
      else
        if (fInspeccion.ValueByName[FIELD_RESULTAD] = INSPECCION_CONDICIONAL) then
        BEGIN
           resultado := 'CONDICIONAL' ;
           QRLabel35.Caption:='VALIDO PARA CIRCULAR HASTA '+TRIM(FECHA_VENC);
           QRLabel36.Caption:='REVERIFICACION GRATUITA HASTA '+TRIM(FECHA_VENC);
         END
        else
        if (fInspeccion.ValueByName[FIELD_RESULTAD] = INSPECCION_RECHAZADO) then
           BEGIN
          resultado := 'RECHAZADO';
           QRLabel35.Caption:='NO APTO PARA CIRCULAR';
           QRLabel36.Caption:='REVERIFICACION GRATUITA HASTA '+TRIM(FECHA_VENC);
           END
        else
        BEGIN
    ...
    //  LblXVencimiento.Caption := fConsulta.FieldByName('VENCIMIENTO').AsString;
     ...
     QRLabel33.Caption:=TRIM(resultado);
    ...
    //cambio por los nuevos informes de inspeccion 19/09/2013
 {
    ...
    //para los nuevos informes
     case StrToInt(aCodigoFrecuencia) of
      ...
      4:   imprimir_tipo_vehiculo := 'TRANSPORTE PUBLICO PERSONAS HASTA 9 PLAZAS';
      5:  imprimir_tipo_vehiculo := 'TRANSPORTE PARTICULAR PERSONAS MAS DE 9 PLAZAS';
      6:  imprimir_tipo_vehiculo := 'TRANSPORTE DE CARGAS';
      7: imprimir_tipo_vehiculo := 'VEHICULO AGRICOLA';
      8: imprimir_tipo_vehiculo:= 'VEHICULO VIAL';
      9:  imprimir_tipo_vehiculo:= 'ESCUELA DE CONDUCTORES';
      10:  imprimir_tipo_vehiculo := 'SERVICIO DE ALQUILER';
      11: imprimir_tipo_vehiculo := 'AMBULANCIA';
      12:  imprimir_tipo_vehiculo := 'TRANSPORTE PARTICULAR PERSONAS MAS DE 9 PLAZAS';
    end;

     QRLabel34.Caption:=TRIM(imprimir_tipo_vehiculo);
      //PLANTA
