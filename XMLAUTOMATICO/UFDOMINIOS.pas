...
{$R *.DFM}
    ...
    var
        ...
        continua:boolean;  patente:String;
    begin
    continua:=true;
        bSalir := FALSE;
               ...
               with TSQLQuery.Create(application) do
                 try
                    ...
                    sql.Add('select * from tinspeccion ti, tdatosturno td  '+
                            ...
                            ' and  ti.inspfina=''S''  '+
                           ' and TI.CODVEHIC =TD.CODVEHIC and TI.RESULTAD=''A''  and td.turnoid='+inttostr(CODTURNOID)) ;
                    Open;
                    while not eof do
                     begin
                              ...
                              if frmturnos.respuestaid_Abrir=1 then
                                begin
                                     ...
                                     if frmturnos.solicitar_reverificacion(CODTURNOID,patente,codinspein)=true then
                                       begin
                                         if fileexists(ExtractFilePath(Application.ExeName) +'Recibidoreve'+inttostr(codinspein)+'.xml') then
                                            begin
                                                if frmturnos.generar_archivo('Recibidoreve'+inttostr(codinspein))=true then
                                                    begin
                                                      frmturnos.codturnoreve:=-1;
                                                      frmturnos.leer_respuesta_reverificacion(ExtractFilePath(Application.ExeName)+'Recibidoreve'+inttostr(codinspein)+'.txt',CODTURNOID,true,'S');
