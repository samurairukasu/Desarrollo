...
procedure TTimerThread.Execute;
  ...
  begin
    Result := (not Assigned(fOwner)) or Terminated or Application.Terminated or (FOwner = nil);
  end;

begin
  repeat
    if not ThreadClosed
    then if SleepEx(FInterval, False) = 0
        then begin
            if not ThreadClosed and Assigned(FOwner) and FOwner.FEnabled
            then begin
              with FOwner do
                if (SyncEvent)
