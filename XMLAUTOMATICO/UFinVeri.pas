...
uses
  ...
  USAGCLASSES,UtilOracle,USAGVARIOS, uUtils, Provider, DBClient, ADODB, uinforme,usuperregistry,uversion;
...
valor:=TRIM(COPY(TRIM(PATENTE),LENGTH(TRIM(PATENTE)),1));
 if (valor='0') or (valor='1') or (valor='2') or (valor='3') or (valor='4')
  ...
  begin
     ...
     ULTIMO:=STRTOINT(TRIM(COPY(TRIM(PATENTE),3,1)));
  end;
...
var
UnaCabecera : tCabecera;  NRO_INFORME,DOMINIO:STRING;
vObleaAsig,NROCERTIFICADO: String;           aqin,aqi:TSQLQuery;
aQ: TSQLQuery;    I,anio,CLASE,cantidadhojasS:LONGINT;    TIMPRE:TIMPRESIONCABA;
imprimirMed: boolean;  cantidadhojas,hojasinforme:longint;
codigoinpeccionseleccionado,icodturnoid:longint;
continuar:boolean;   aQim:TSQLQuery;
begin
  ...
  Begin
   ...
   aqin.EXECSQL;
     ...
     ////armar consulta
      ...
      ///fin consulta
        ...
        if fInspeccion.ValueByName[FIELD_TIPO][1] in [T_NORMAL, T_REVERIFICACION, T_VOLUNTARIA, T_VOLUNTARIAREVERIFICACION] then
          ...
          try
            sqlconnection:=MyBD;
            Storedprocname:= 'Pq_finalizar.DatoImprimeDefecto';
            parambyname('acodinspe').value:=codigoinpeccionseleccionado ;
            parambyname('aejercicio').value:=anio;
            parambyname('acodclase').value:=CLASE;
            execproc;
           ...
           { MENSAJEIMPRESION.Label1.Caption:='IMPRIMIENDO INFORME DE INSPECCION...';
            MENSAJEIMPRESION.}
          ...
          {------------------impresion----------------------}
                ...
                { MENSAJEIMPRESION.Label1.Caption:='IMPRIMIENDO INFORME DE INSPECCION...';
            MENSAJEIMPRESION.};
         //   cantidadhojasS:=CALCULA_CANTIDAD_HOJA(codigoinpeccionseleccionado,anio);
          ...
          label2.Caption:='CANTIDAD: '+inttostr(cantscaneeo)+' de '+inttostr(hojasinforme);
           ...
           begin
            ...
            if ((LabelApta.Visible) or  ((entrega_oblea_a_condicionales=TRUE)and (LabelCondicional.Visible))) then
             ...
             if Application.MessageBox( '¿Desea cencelar el formulario ?', 'Cancelar',
               MB_ICONQUESTION OR MB_YESNO ) = ID_YES then
               BEGIN
               ...
               while not  RXMEMORYDATA1.Eof DO
                BEGIN
                  ...
                  aqi.Close;
                  aqi.SQL.Clear;
                  aqi.SQL.Add ('update tcertificados set estado=''S'' '+
     ...
     // fin oblea
          ...
          try
            sqlconnection:=MyBD;
            Storedprocname:= 'Pq_finalizar.BORRARTMP';
            parambyname('acodinspe').value:=codigoinpeccionseleccionado ;
            execproc;
