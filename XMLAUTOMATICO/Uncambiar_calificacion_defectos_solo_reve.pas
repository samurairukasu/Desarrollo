...
if trim(edit1.Text)='' then
 ...
 begin
     Application.MessageBox( 'Debe seleccionar la Calificación del Defecto.',
  'Atención', MB_ICONSTOP );
...
if Application.MessageBox( pchar('¿Desea modificar la calificación del defecto: '+trim(cadena)+' del vehiculo: '+trim(patente)+' ?'), 'Modificar Calificación',
  MB_ICONQUESTION OR MB_YESNO ) = ID_YES then
  begin
      t:='E';
      f:='N';
      sql1:=' select max(CODINSPE) from TINSPECCION  I, TVEHICULOS V  '+
            ' where I.CODVEHIC=V.CODVEHIC AND TIPO =:tipo AND INSPFINA=:fina  '+
