...
bandera:=true;
 ...
 begin
   showmessage('Debe Ingresar el Código de Inspección.');
  exit;
 end;
 ...
 bandera:=true;
   ...
   modulo.sql_global.ExecSQL;
    ...
    mensaje.close;
 APPLICATION.ProcessMessages;
   texto:='Cambia la patente  '+patenteA+' por la patente '+patenteN+' a la inspección '+inttostr(codinspe);
 ...
 APPLICATION.ProcessMessages;
  showmessage('Proceso Terminado.');
     texto:='Cambia la patente  '+patenteA+' por la patente '+patenteN+' a la inspección '+inttostr(codinspe)+' con cod vehic: '+inttostr(codvehic);
  modulo.guardar_incidencia(texto);
